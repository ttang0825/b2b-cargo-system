...
마이그레이션 1). 커밋 7개. 🔴 **DB 변경 있음** — `quotes` 에 nullable 2컬럼
...
**═══ ⑫ ⚠️ 다음 차수가 알아야 할 것 ═══**
🔴 **다음은 29차(포털 조회 흐름) + P0-1 + P0-2 + P1-1이고, 시안이 있어야 착수한다.**
...
- ~~**28차: 내부관리시스템 연동 조사**~~ — **58차 세션에서 완료**(보고서
  ...
  - 🔴 **P0 는 둘뿐이다** — ① 관리자 화주요청 상세 펼침(품목·상하차조건·물품특성·담당자 6필드)
    ② 배차 상세에 오더 `special_notes`. **둘 다 DB 변경 0** 이라 29차 PR 에 얹는다
...
- 🔴 **다음 지시서는 29차(포털 조회 흐름 + P0 2건)이고 시안이 있어야 착수한다.**
