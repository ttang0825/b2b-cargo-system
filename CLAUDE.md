...
**작성일: 2026-07-23** (최종 갱신: 2026-08-13, Claude Code 세션에서 직접 갱신 — 28차
...
2차 긴급 안전 수정 → 3차 로고·파비콘까지 완료), 사용자가 작업지시서를 차수별로 전달하는
방식으로 진행 중입니다. **작업지시서 차수와 세션 차수가 다르므로 헷갈리지 말 것**
(1차=25차 세션, 2차=26차 세션, 3차=27차 세션):

- ~~1차(사전조사)~~ / ~~2차(모바일 오버플로우·색인차단·메타)~~ / ~~3차(로고·파비콘)~~ /
  ~~4차(용어 정리·메뉴 개편·`/about`·`/vehicles` 신규)~~ — 완료
  (**4차 범위가 도중에 바뀌었음** — 원래 4차였던 "법적 문서+푸터"가 5차로 밀림)
- **5차(다음): 법적 문서 페이지 3종(`/terms`, `/privacy`, 이메일무단수집거부) + 푸터
  재구성.** `app/robots.ts`에 `/terms`·`/privacy` Allow가 이미 들어가 있고(생성 전까지는
  404일 뿐), 이메일무단수집거부 경로를 정하면 robots에 추가해야 함.
  **푸터 링크 목록에 `/about`·`/vehicles`를 반드시 포함할 것**(4차에서 신설된 페이지).
  푸터 로고는 넣지 않는 것이 기본이되, 넣는다면 3차에서 만든 `components/BrandLogo.tsx`를
  재사용할 것(새 이미지를 만들지 말 것 — 이 컴포넌트는 훅을 안 써서 서버 컴포넌트에서도 씀).
  약관·처리방침 본문의 용어도 "고객"으로 통일하되, **정의 조항에 "관계 법령상 '화주'에
  해당합니다"를 병기해야 함**(병기 없이 "화주"만 지우면 상법상 운송주선 규정과의 연결이
  끊겨 법령 해석 시 다툼의 여지가 생김). **"차주"는 바꾸지 않음**, 법령 인용문 안의
  "화주"도 원문 그대로 둘 것.
  **⚠️ 착수하려면 별첨 문서 3종이 필요함**(작업지시서에 명시된 첨부인데 아직 전달 안 됨):
  `개인정보처리방침_v2_최종안.md`, `이용약관_초안_v1.md`,
  `랜딩페이지_공개준비_인수인계문서_v1.md`(4-1·4-4절 — 푸터 시안과 이메일무단수집거부 문안).
  이 문서들 없이는 약관·방침 본문과 푸터 시안을 지어낼 수 없으므로 반드시 받고 시작할 것.
  이미 확정된 사용자 결정: (a) 운임기준표를 홈페이지에 공개하지 않음 → 약관 제10·11·16조의
  "홈페이지에 게시한 기준표" 인용을 지우기만 하면 추가비·대기료 청구 근거가 사라지므로
  **"견적 시 개별 고지"로 근거를 옮겨야 함**(제11조에 고지 조항 신설), (b) 첫 거래 혜택은
  약관에 넣지 않음(한시 프로모션), (c) **적재물배상책임보험은 가입 사실만 명시하고 담보
  한도·범위는 적지 않음** — 제19조 2항에서 금액을 빼되 "요청 시 안내한다"는 문장을 반드시
  함께 넣고, 4항(고의·중과실 예외)은 그대로 유지할 것. **보험 문구는 여기까지만** —
  담보 범위나 "모든 화물이 보상된다"는 취지의 표현을 어디에도 추가하지 말 것(사용자가
  현재 담보 범위를 별도로 확인·개선 중이라 결과에 따라 문구가 바뀔 수 있음)

  **⚠️ 5차 착수 전 사전조사 결과(29차 세션에서 조사만 수행, 코드 변경 없음)** — 별첨
  문서가 또 전달되지 않아 페이지 구현은 못 했지만, 지시서가 요청한 확인 4건을 처리함:
  ...
  3. **외부 확인 2건 불가** — 공정위 사업자정보 페이지(`ftc.go.kr`)와 Vercel
     ...
     (프록시 403). 지시서 지침대로 **공정위는 링크 없이 텍스트만 표기**, **Vercel
     문의처는 비워둔 채 진행**할 것. 사용자가 값을 확인해 주면 채워 넣으면 됨.
...
- **6차(그 다음): 동의 절차 구현. 착수 전 3가지를 사용자와 먼저 확정할 것** — (a) 동의문구
  ...
  (회사 vs 담당자·연락처)** — 26차 조사에서 수신번호 출처가 3군데로 갈리고 한 화주에
  포털 계정이 여러 개일 수 있음이 확인돼, 회사 단위로만 두면 담당자별 동의를 표현할 수
  없음(위 26차 세션 기록 참고), (c) 조사 중 드러난 그 밖의 구조적 제약
- 그 밖에 아직 범위 밖인 것: **SMS 문구 8종(`lib/sms/templates.ts`)과 견적서 PDF의 용어
  정리**(4차에서 고객 접점 용어를 "고객"/"운송관리"로 바꿨지만 이 둘은 범위 밖이라 여전히
  "화주" 표현이 남아 있음 — 고객에게 그대로 전달되는 문서라 정리 대상), **`/vehicles`에
  냉장·냉동 등 나머지 차량형태를 노출할지 여부**(현재 카고/탑차/윙바디/리프트 4종만 노출),
  FAQ(보류), 데스크탑 섹션 패딩 목표치(96px) 미달, 견적서 PDF에
  대표번호·로고 미표시(번호 확정 후), 관리자·화주포털 헤더 로고(랜딩만 적용됨),
  OG 공유 이미지(파비콘과 별개로 1200×630 가로형 필요), SEO 일괄(도메인 확정 후:
  `metadataBase`/canonical/Open Graph/OG 이미지/sitemap.xml/서치콘솔 등록), 성능 점검
  (Lighthouse mobile, LCP/CLS/INP, Hero 이미지 `next/image`)
- **4차 완료 후 확인이 필요한 운영상 파급**: 기준표를 공개하지 않고 추가비 근거를
  "견적 시 개별 고지"로 옮기면, **견적서에 무료 대기시간·대기료·회차비 조건이 실제로
  표기되어야 약관이 성립함.** 현재 견적서 PDF(`app/admin/quotes/[id]/print`,
  `app/customer/quotes/[id]/print`)에 이 항목이 있는지 확인하고, 없으면 별도 세션
  과제로 등록할 것
...
6. **SMS 문구·발송 방식 정리** — 23차 세션(SMS 발신 연동)에서 8종 이벤트 문구와
   "발송 직전 확인 모달" 방식까지는 구현했지만, 사용자가 문구 표현·발송 방식
   자체를 좀 더 다듬고 싶어함(구체적인 요구사항은 아직 미정 — 다음 세션에서
   사용자에게 먼저 무엇을 바꾸고 싶은지 확인부터 할 것). 24차 세션(상차지/하차지
   담당자 정보 추가) 완료 시점에 사용자가 "차후 작업으로 미룬다"고 명시적으로
   확인한 항목
