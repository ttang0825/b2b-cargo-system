...
> 🔴 이 문서(`CLAUDE.md`)는 **185,273자**입니다(2026-09-14 문서 정합 3차 직후) — 컨텍스트
> 예산 판단에 쓰는 값입니다. ⚠️ 그 작업 **직전**에는 177,701자였습니다.
...
| `docs/history/current.md` | 33차 #139 2026-09-10  →  거래처 온보딩 `/guide` #148 2026-09-14   (14건) | 74,184자 |
...
14. **관리자 메뉴는 3개 그룹 드롭다운 구조**: 화주 확보(화주관리·화주신청·공개문의) /
    화주 관리(활성화주CRM·화주요청) / 운송 운영(운임기준표·견적·오더·**배차·정산·차주**·외부정보망).
    새 관리자 메뉴 추가 시 `TopNav.tsx`의 `NAV_GROUPS`에 적절한 그룹으로 넣을 것.
    ⚠️ **운송 운영 순서가 35차 리뷰 4라운드에 바뀌었다**(PR #146 `1d0b14b`, 사용자 지시
    *"차주관리는 정산관리 아래에 위치하게"*) — 그전에는 「…오더·**차주**·배차·정산」이었다.
...
```
  ...
  36차  내부 정합 ② 화주·차주 (CRM 항목 신설 + 거래조건이 여기다)   ← 다음
...
🟢 **35차는 2026-09-14 에 끝났다**(PR #146). 🔴 **다음은 36차(내부 정합 ② 화주·차주)이고,
그 뒤가 24시콜 가져오기 ⓐ→ⓑ→ⓒ 세 차수다**(설계 확정 · 35차 merge 를 기다리던 것).
...
```
  ...
  🟢 프리렌더 기준선     **45** (2026-09-14 실측 · `npm run build` 로 직접 셌다).
                      ⚠️ **44 → 45 는 `/guide` 하나가 늘어난 것이다**(항목을 `diff` 로 갈랐다).
