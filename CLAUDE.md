...
**작성일: 2026-07-23** (최종 갱신: 2026-08-20, Claude Code 세션에서 직접 갱신 — 37차
...
**11차 레이아웃 전면 개편·헤더 옐로까지 완료**), 사용자가 작업지시서를 차수별로 전달하는
방식으로 진행 중입니다.
**작업지시서 차수와 세션 차수가 다르므로 헷갈리지 말 것**
(1차=25차 세션, 2차=26차, 3차=27차, 4차=28차, 5차=30차, 6차=31차, 7차=32차, 8차=33차,
9차=34차, 10차=35차, 11차=37차 세션 — 29차는 별첨 미전달로 5차 사전조사만 수행한 세션,
36차는 지시서 없이 사용자 요청으로 진행한 문자 이력 화면).
⚠️ **11차 지시서가 도착했을 때 이 로드맵에는 이미 "11차 = 동의 절차"가 예약돼 있어 번호가
겹쳤음** — 레이아웃 개편을 11차로 확정하고 동의 절차를 12차로 밀었으니, 다음 지시서가
"12차"로 오는지 확인할 것:
...
- ~~**5차: 법적 문서 페이지 3종(`/terms`, `/privacy`, `/email-policy`) + 푸터 재구성**~~ —
  **30차 세션에서 완료**(상세 내용은 위 30차 세션 기록 참고). 아래는 이후 세션이 알아야 할
  ...
  - **시행일은 자리표시자**(`lib/legalInfo.ts`의 `LEGAL_EFFECTIVE_DATE`, 2026-09-01) —
    실제 공개일이 정해지면 이 값과 `LEGAL_EFFECTIVE_DATE_IS_PLACEHOLDER`를 **같이** 바꿀 것
    (약관 부칙·처리방침 3곳·이메일무단수집거부 게시일 총 5곳이 이 상수 하나를 참조)
  - **Vercel 개인정보 문의처는 아직 비어 있음**(`VERCEL_PRIVACY_CONTACT`) — 처리방침
    제6조에 "확인 중"으로 표시됨. 사용자가 값을 알려주면 이 상수만 채우면 됨
...
- **12차(다음): 동의 절차 구현. 착수 전 3가지를 사용자와 먼저 확정할 것** — (a) 동의문구
