...
**작성일: 2026-07-23** (최종 갱신: 2026-08-25, Claude Code 세션에서 직접 갱신 — 44차
...
제약). **Preview에서 거리 `10.4` · 톤수 `1톤` → 50,000원을 사용자가 확인할 항목이다.**
...
**15차 거리 구간 매칭 버그 수정까지 완료**. 그 사이 **차수 없는
...
9차=34차, 10차=35차, 11차=37차, 12차=39차, 13차=40차, 14차=43차, **15차=44차** 세션 — 29차는
...
붙인 결과가 아래이며, **15차까지 전부 끝났다** — 🔴 **다음은 16차이며, 지시서 부록 B가 예고한
순서(16차 운임값 교체 → 17차 차급 9종 → 18차 가산기준 조정)를 지킬 것**:
...
- 🔴 **가장 먼저 — 다음 지시서는 16차다.** 15차 부록 B가 예고한 순서:
  **16차** `base_fare` 90칸 교체 + `/vehicles` 공개 시작가 + `rate_vehicle_extra_fees` 2행 +
  첫거래지원 할인 제거 / **17차** 차급 9종 확장(배열 분리 + 로컬 재정의 정리 + 45행 INSERT) /
  **18차** 가산기준 조정(파손주의 · 장척/중량 · 상차 시각 지정).
  ⚠️ **16차는 운임값을 실제로 바꾸는 첫 차수다** — `/admin/rates` 화면은 **숫자를 클릭하는
  순간 저장되고 되돌리기가 없으며**(이력 테이블이 없다) 팝업이 저장보다 나중에 뜬다.
  15차가 로직을 먼저 고쳐둔 덕분에, 16차에서 계산이 안 되면 **데이터 탓임이 확정된다.**
- ⚠️ **사용자가 Preview에서 확인할 것(15차 잔여)**: `/admin/quotes`에서 거리 `10.4` ·
  톤수 `1톤` → 기본운임 **50,000원**, 그리고 근거리 주소 두 개로 자동 거리계산이 소수를
  반환할 때도 계산이 뜨는지. 이 환경에서는 자격증명·프록시 제약으로 확인할 수 없다.
