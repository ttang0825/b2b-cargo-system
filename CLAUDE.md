...
**DB 변경 없음.** 신규 파일 1개(`lib/insuranceInfo.ts`). 커밋 5개로 분리함.
...
**(6) 신규 상수 `lib/legalInfo.ts`** — `LEGAL_EFFECTIVE_DATE`(자리표시자 2026-09-01,
약관 부칙·처리방침 시행일/최종개정일/부칙·이메일무단수집거부 게시일 **총 5곳**이 참조)/
...
않음), Vercel 개인정보 문의처도 확인 불가라 처리방침 제6조에 **"확인 중"으로 표시**되며 빌드
경고 대상임(사용자가 값을 알려주면 `VERCEL_PRIVACY_CONTACT` 한 곳만 바꾸면 됨).
