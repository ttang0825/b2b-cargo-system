...
**작성일: 2026-07-23** (최종 갱신: 2026-08-20, Claude Code 세션에서 직접 갱신 — 39차
...
11차 레이아웃 전면 개편·헤더 옐로 → **12차 정합성·상수·진입경로·이미지 구조까지 완료**),
...
9차=34차, 10차=35차, 11차=37차, 12차=39차 세션 — 29차는 별첨 미전달로 5차 사전조사만 수행한
세션, 36차는 지시서 없이 사용자 요청으로 진행한 문자 이력 화면, **38차는 12차 사전조사
전용 세션(코드·DB 무변경)**).
⚠️ **동의 절차가 두 번 밀렸다.** 원래 11차로 예약돼 있었는데 11차 지시서가 레이아웃 개편으로
와서 12차로, 12차 지시서가 정합성 작업으로 와서 다시 **14차**로 밀렸다. 실행 순서대로 다시
붙인 결과가 아래이며, 다음 지시서가 **"13차"**로 오는지 확인할 것:
...
- **13차(다음): 랜딩 섹션 신설 — 시안 확정 후 전달 예정.** 운송관리 섹션(⑤ 곁다리 카드를
  독립 섹션으로 승격 + 버튼 2개), 안전·책임 섹션(`INSURANCE_ENABLED`로 제어), 헤더 개편
  ("로그인" 버튼화·"문의·신청 현황" 제거 — 푸터가 그 자리를 대신하도록 12차에 미리 넣어둠),
  「선택 이유」 3·4번 축약, 히어로 문구 ↔ 견적 폼 정합.
  ⚠️ **360px 헤더에는 눈에 보이는 요소를 더 넣을 자리가 없다**(38차 실측: 랜딩 여유 15px,
  `/quote` 11px — 27차 기록의 24px는 햄버거가 들어오기 전 값이라 무효). 데스크탑은 여유가
  크다(1280에서 551px, 820에서도 151px). 데스크탑 메뉴↔햄버거 전환 분기점은 **760px**
...
- 🔴 **랜딩 이미지 5종 교체 — 사용자 작업이며 Go-Live 차단 항목**(37차). 규격은 37차
  기록의 (5)번 표에 있고, 파일이 준비되면 `lib/landingImages.ts`만 채우면 됨.
