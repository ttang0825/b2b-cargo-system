...
> 🔴 이 문서(`CLAUDE.md`)는 **202,371자**입니다(2026-09-15 36차 PR 2 #151 직후) —
> 컨텍스트 예산 판단에 쓰는 값입니다. ⚠️ 그 직전에는 195,736자, 그 앞에는 191,288자였습니다.
...
| `docs/history/current.md` | 문서 정합 정리 2차 2026-09-11  →  36차 PR 2 #151 2026-09-15   (8건) | 62,892자 |
...
6. **상하차 일시 규칙**: 하차일시는 상차일시보다 최소간격 이후만 선택 가능하고,
   ...
   ```
     ...
     운송오더               상차 +2시간  🟢 안 바꿨다
   ```
   🔴 **세 값 중 30분 두 곳은 `lib/dropoffGap.ts` 한 정의처를 쓴다** — 화면에 숫자를 다시
   적지 말 것(그래서 갈렸었다). 🔴 **`calcMinGapHours()`(거리기반)를 되살리지 말 것** —
   10km 건에도 2시간이 강제돼 실제 도착 시각보다 뒤로만 적을 수 있었다.
   🔴 **당착·내착은 예외다**(27차) — 시각이 무관한 선택지라 규칙을 걸면 23:40 상차 건의
   접수가 막힌다. 그 판단은 화면이 한다.
   🟢 **36차 PR 2 에 견적관리에도 「지금·당착·내착」 칩이 생겼다** — 공용
   `components/DateTimePicker.tsx` 의 **opt-in prop** 이고 **기본값이 전부 꺼짐**이라
   운송오더 2화면·랜딩 폼은 안 바뀐다. 🔴 **기본값을 켜지 말 것.**
...
```
  ...
  37차  내부 정합 ② 화주·차주 (CRM 항목 신설)   ← **다음**   🔴 **거래조건은 36차가 가져갔다**
  ——   24시콜 가져오기 ⓐ → ⓑ → ⓒ (설계 확정 · 35차 merge 후 착수 — 아래 블록)
...
```
  ...
  🔴 하차 +30분     `lib/dropoffGap.ts` 하나를 **세 화면**이 쓴다. 🔴 `calcMinGapHours()`(거리기반
  ...
  🔴 일정 칩 opt-in `DateTimePicker` 의 「지금·당착·내착」은 **기본 꺼짐**이다 — 운송오더 2화면·
                  랜딩 폼이 같은 부품을 쓴다. 🔴 **기본값을 켜지 말 것.** 🔴 포털 부품을
  ...
  🔴 공유 링크 열쇠  `/q/[token]` — **토큰이 곧 열쇠다**(22자 base62 ≈ 131비트, 길이 줄이지 말 것).
                  🔴 **주소만으로는 아무것도 안 보인다** — 뒤 4자리 확인 **뒤에** 처음 받아온다
                  (실측 HTML 에 견적 데이터 0건). 🔴 **서버 컴포넌트로 바꾸면 이 성질이 무너진다** ·
                  🔴 **API 는 POST**(GET 이면 토큰·4자리가 주소창·로그·리퍼러에 남는다) ·
  ...
  ⚠️ 4자리는 방지턱  10,000 가지뿐이라 **토큰을 아는 사람에게는 벽이 아니다**(32차 로그인 제한과
                  같은 성격·같은 부품). 진짜 방어선은 토큰이다. 막으려는 것은 「문자가 전달됐을 때
                  무심코 열리는 것」이고 그건 확실히 막는다
...
🔴 **24시콜 가져오기 설계서(v3, 2026-09-11)와의 대조 — 36차보다 먼저 읽을 것**
...
```
  ...
  🟢 프리렌더 기준선     **46** (2026-09-14 실측 · `npm run build` 로 직접 셌다).
