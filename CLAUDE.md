...
요율은 관리자 수정 가능한 설정 테이블(`insurance_rate_settings`)로 분리 완료·merge됨
+ 직원 계정
...
- **차주 수금/지급 운임 표시 구조 — 3차 세션 보정(산재보험료 계산 로직
  ...
  `customer_charge_vat_included`(화주청구금액 부가세 포함여부, 기본 true —
  ...
  (관리자가 `/admin/settings/insurance-rate`에서 수정 가능, 원칙 40번)에서
...
40. **정부 고시로 매년 바뀌는 요율(산재보험료율, 필요경비공제율 등)은 코드에
    ...
    수정 화면(`/admin/settings/insurance-rate`)은 운임기준표·외부정보망 관리와
    동일하게 조회는 누구나, 수정은 관리자만(원칙 25번 이중 체크) 가능하도록
    구현. 비슷하게 "이 값은 언젠가 법령/고시가 바뀔 수 있다"는 값을 새로 다룰
...
- **차주 수금/지급 운임 표시 구조 — 3차 세션 보정(산재보험료 계산 로직 수정)**:
  ...
  운영" 그룹)도 신규 추가(원칙 25번 화면단+서버단 이중체크). **사전확인
