...
커밋 9개(본작업 6 + **실사용 리뷰 3라운드 3**). **PR #105.**
🟢 **DB 스키마 변경 없음** · 🟢 **신규 의존성 0** · 🟢 **관리자 화면 파일 변경 0**
(견적서 print 한 쪽만 예외 — 아래 ⑥).
...
⚠️ **하차의 「당착/내착」과 발주요청 정산방식은 이번에 안 했다** — 둘 다 `portal_order_requests`
에 **컬럼이 없어서**(실측: 정산 3필드 없음, 35컬럼) 마이그레이션이 필요하다. 27차 지시서가
DB 변경을 금지해서 **물어두고 멈췄다.** 🔴 **"왜 안 되어 있지" 하고 헤매지 말 것.**
...
```
  ...
  모바일 390px             가로 오버플로우 0(접힘·펼침 둘 다) · 운임 내역 2열 · 화물 1열 ·
                          「견적서 상세 보기」가 맨 위 전체 폭(328px)
  globals.css              origin/main 대비 **850 추가 / 0 삭제** · 새 선택자 전부 `.pv2-*`
  npm run build            프리렌더 실패 48개가 origin/main 기준선과 **항목 단위 완전 동일**
  npx tsc --noEmit         통과(출력 0줄) · package.json·package-lock.json 변경 0
```
⚠️ **관리자 렌더링 대조는 하지 않았다** — 관리자 파일 변경 0건이고 `globals.css` 는 **삭제 0**에
새 선택자가 전부 `.pv2-q*`(견적 확인 한 화면만 쓴다)라 전역에 닿는 경로가 없다.
🔴 **다음에 `.portal-v2` 서체·포커스처럼 넓게 걸리는 것을 건드리면 9화면 × 12지표를 다시 잴 것.**
...
🟢 **28차가 쓸 실측을 미리 떠뒀다** — `portal_order_requests` **35컬럼**(마지막이 `loading_type`,
NOT NULL) · `customer_locations` **11컬럼** · 그 표에는 **status CHECK 제약이 없다**(값은 코드가
쓰는 문자열이고 실제 분포는 대기중 2 · 승인됨 1 · **반려 0**).
🔴 **28차가 꼭 볼 것 하나 더 — 화주 「견적 승인」이 관리자 화면에 어떻게 보이는가.**
지금은 `수주` 로만 보이고 누가 바꿨는지 흔적이 없다(위 ③).
...
```
⚠️ **답을 못 받은 것 1건**(26차부터) — *"자주 쓰는 화물을 불러오면 그 목록 이름이 어딘가
표시되면 좋겠다"*. ⚠️ **사용자 확인 대기 1건** — 무료 대기시간의 `"초과 시 30분당"` 단위.
⚠️ **사용자가 코드 밖에서 할 일: 없다.**
