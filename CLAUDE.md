...
> 🔴 이 문서(`CLAUDE.md`)는 **195,736자**입니다(2026-09-15 36차 A·B장 PR #150 직후) —
> 컨텍스트 예산 판단에 쓰는 값입니다. ⚠️ 그 직전에는 191,288자, 그 앞에는 185,273자였습니다.
...
| `docs/history/current.md` | 문서 정합 정리 2차 2026-09-11  →  36차 #150 2026-09-15   (7건) | 51,508자 |
...
6. **상하차 일시 규칙**: 하차일시는 상차일시보다 최소간격 이후만 선택 가능하고,
   ...
   말 것.** ⚠️ 다만 견적 등록 화면의 **안내 문구는 아직 「현재 시각 이후로만 선택 가능합니다」**
   라 실제 동작과 어긋난다 — 36차 D장이 하차만 다루기로 해서 **일부러 안 고쳤다.**
...
```
  ...
  ——   36차 PR 2 (C·D·E장 · DB 0)   ← **다음**
        C장 활성 화주 목록의 선착불 미수금 — 🟢 원인을 갈랐고 **코드 결함 하나**다(데이터는 이미 맞다) ·
        D장 견적관리 하차 일시 = 상차 +30분부터 · E장 「최종금액 직접 입력」 → 견적서 「조정」 줄.
        🔴 그리고 **위 세금계산서 누락 복구**가 첫 커밋이다(DB 0)
  37차  내부 정합 ② 화주·차주 (CRM 항목 신설)   🔴 **거래조건은 36차가 가져갔다**
