...
세션: "화주포털 화면 ② — 발주 흐름(25차)" — 신규 파일 3개
(`lib/customerPresets.ts` · `components/pv2/Pv2AddressField.tsx` · `Pv2DateTimeField.tsx`
+ `components/pv2/` 폴더 자체). 커밋 5개. 🔴 **DB 변경 없음.**
🟢 **관리자 회귀 0** · 🟢 **시안 소스 값 대조 13항목 불일치 0**(24차에 없던 검증).

**═══ ① 🔴 25차가 셋으로 쪼개졌다 — 26·27차가 남았다 ═══**
화면 7개를 한 차수에 넣으면 리뷰가 감당되지 않는다(24차는 화면 하나였는데도 실사용 리뷰
3라운드에 시안 불일치 6곳). 사용자 확정 2026-08-27.
```
  25차 ✅  발주 흐름 — 발주 요청 · 배송지·화물 관리 · 프리셋
  26차     견적 흐름 — 견적 목록 · **상세 신규** · 견적서 print 2종(부가세 행) · 입금 계좌
  27차     조회 흐름 — 배차·운송 조회 + **홈 배지 3단계** · 정산 · 월별 통계(+캘린더) · 담당자 정보
```
...
원인: 조사 세션이 **909×540 뷰포트 JPEG** 로 시안을 봐서 아래쪽을 못 봤다(보고서 부록 B가
그 한계를 스스로 적었다). 🔴 **fullPage 렌더링이 정본이다.** 접이식 블록을 만들지 않았다.
⚠️ **다음에 시안을 근거로 삼을 때는 fullPage 인지부터 확인할 것.**

**═══ ③ 🔴 상하차조건은 8종이다 — 시안 6종으로 합치지 말 것 ═══**
시안은 `기본운송·수작업·지게차·호크(크레인)·도크·기타` 6종으로 그렸지만 **DB 8종 그대로**
노출한다(사용자 확정 2026-08-27). 실측: DB `rate_surcharges` 8행 · `lib/loadingMethods.ts`
8종 · 라벨 집합 완전일치.
```
...
```
⚠️ 드롭다운 항목 2개 추가라 **시안 레이아웃에 영향이 없다.** 순서는 `loadingMethods.ts`
배열 순서(`도크` 는 `지게차` 다음)를 따른다.

**═══ ④ 🔴 20차 컬럼 5개와 customer_presets 가 여기서 처음 쓰였다 ═══**
20차가 만들어두고 **사용처가 0곳**이던 것들이다. 어느 칸에 대응하는지:
```
  customer_locations   location_name    배송지 이름 (예: 가산 본사 창고)
                       address_detail   상세주소
                       contact_name     담당자 이름
                       contact_phone    전화번호
                       notes            특이사항
  customer_presets     preset_type='cargo'    자주 쓰는 화물
                       preset_type='request'  자주 쓰는 요청
```
🔴 **`name`·`note` 를 새로 만들지 말 것** — 마이그레이션 파일 안의 단언이 막고 있고,
관리자 화면 2곳(`admin/companies/[id]:412` · `admin/quotes:380`)이 이미 `location_name` 을
select 에 넣고 있어 만들면 **같은 배송지에 이름이 둘** 생긴다.
⚠️ **20차 이전 행은 `location_name` 이 null 이라 주소로 폴백해 그린다** — 안 그러면 기존
12행의 카드 제목이 통째로 비어 보인다. 목 데이터에 그 상태의 행을 넣어 실측했다.
🔴 **`customer_presets` 를 둘로 쪼개거나 `payload` 를 컬럼으로 펼치지 말 것**(20차 확정).
재직 계정 조건은 **DB 정책이 조회·저장 양쪽에 걸고 있다** — 정책을 손대지 말 것.

**═══ ⑤ 🔴 프리셋 payload 는 저장 시점 스냅샷 — 없는 값은 조용히 버린다 ═══**
20차가 *"21차에서 처리할 것"* 으로 남긴 처리를 여기서 했다(`sanitizeCargoPayload()`).
옵션 이름이 바뀌면 payload 값이 지금 목록에 없을 수 있는데, **오류를 던지면 프리셋 하나
때문에 폼 전체가 막힌다** — 화주는 원인을 알 수 없고 그 프리셋을 지우기 전엔 발주를 못 한다.
🟢 **실측했다** — payload 에 `없어진특성`·`기사도움`·`2인수작업` 을 일부러 넣은 표본을
불러오니 그 셋만 버려지고 톤수·차량형태·품목이 채워졌으며 **오류 표시 0**.

**═══ ⑥ 🔴 시안에 없지만 일부러 남긴 것 둘 ═══**
① **제3자 제공 동의**(47차) — 빼면 처리방침 제4조 약속을 어긴다(*"실제 운송 접수 시점에
별도의 동의를 받은 후에만"*). 상·하차지 담당자의 성명·연락처는 화주 본인이 아니라 **제3자의
개인정보**다. 접수 후 자동 해제되는 현행 동작도 유지했다(동의는 발주 건별이다).
...
날짜로 발주할 수 있었다.** 하차에는 "상차 +2시간" 이 있었지만 상차 자체엔 제한이 없었다.
```
  하한  max(오늘, LEGAL_EFFECTIVE_DATE)   🔴 날짜를 하드코딩하지 말 것 —
                                           시행일 전 날짜엔 적용될 약관이 없다
  상한  하한 + 30일
  막는 곳 셋  input min/max · 「오늘」「내일」 칩 disabled · 제출 직전 검증
```
🔴 **칩을 빠뜨리면 구멍이 남는다** — 칩이 `min` 을 우회해 과거 날짜를 그대로 넣는다.
🔴 **제출 직전 검증도 필요하다** — 화면 검증은 콘솔에서 우회된다(원칙 25번). 실측으로
`2020-01-01` 을 강제 주입해 **POST 0건**으로 막히는 것을 확인했다.
⚠️ **지금은 「오늘」·「내일」 칩이 둘 다 비활성이다**(오늘 2026-08-27 < 시행일 2026-09-07).
**의도된 동작이며 9/7 이후 정상 동작한다** — 버그로 보고 걷어내지 말 것.

**═══ ⑧ 🔴 공유 컴포넌트를 고치지 않고 포털 전용 래퍼를 만들었다 ═══**
지시서 §3-3 의 ②안을 택했다. 사유를 남긴다 — **①안(스타일만 덮기)이 불가능했다.**
```
  AddressSearch    포털 밖 6개 파일. 주소검색 버튼·상세주소에 **인라인 style** 이 박혀 있어
                   (padding:0 10px · borderRadius:6 · fontSize:12 · marginTop:6)
                   CSS 로 덮을 수 없다. !important 로 억지로 덮거나 원본을 고치면 6곳이 같이 바뀐다
  DateTimePicker   포털 밖 4개 파일. 전역 클래스(.field·.btn·.btn-ghost)를 쓰고
...
완료조건이 "구조·수치"만 재고 **"시안과 같은가"를 안 쟀기 때문**이다. 그래서 시안
`.dc.html` 에서 값을 뽑아 `getComputedStyle` 과 대조하는 검증을 새로 만들었다.
```
  시안 소스 대조 13항목      불일치 0
    블록 카드 radius 20px·padding 24px 28px / 번호 원 22px·#FFD833·13.5px/700 /
    ① 2열 minmax(280px,1fr) gap 24px / 열 카드 #F4F3EF·radius 16px·padding 20px /
    ② 4열 gap 14px 16px / ③ minmax(300px,1fr) gap 20px 24px /
    제출 21px/800·padding 22px 68px·radius 16px / 카드 그리드 gap 12px /
    저장 카드 radius 20px·padding 18px 22px / 추가 버튼 #1A1A1A·15.5px/600·radius 9px /
    배지 상차지 #FFF9D6·#7A5F00 · 하차지 #EBEAE7·#1A1A1A / 입력 1px #EBEAE7·radius 12px
  폼 실동작 5건            ⇄ 6칸 전부 맞바뀜 · 배송지 불러오기(상세·담당자·연락처까지) ·
                           옛 프리셋 3값 조용히 버림(오류 0) · 동의 미체크 POST 0건 ·
                           과거 날짜 강제 주입 POST 0건
  옵션 수                  상차·하차조건 각 8개(라벨 완전일치) · 희망 톤수 11개 · 블록 4개
  관리자 회귀              4화면 × 11지표 before/after **불일치 0**
  포털 11화면 × 2뷰포트     22장 · 오버플로우 0px · 에러 0 · "불러오는 중" 멈춤 0
  npm run build            프리렌더 실패 48개가 origin/main worktree 와 항목 단위 완전 동일
  npx tsc --noEmit         통과(출력 0줄) · package.json·package-lock.json 변경 0
  globals.css              삭제된 줄 **0**(순수 추가) · 전역 클래스 정의 변경 0건
  마이그레이션              변경 0건 · 안 건드리는 화면 5개 변경 파일 0
```
⚠️ **`.dc.html` 값을 뽑을 때 `grep -o` 로 조각을 자르지 말 것** — 54차가 `…2px;bottom:10px`
를 `-12px` 로 잘못 읽어 음수 offset 을 넣었고 카드가 `overflow:hidden` 이라 잘렸다.
**여는 태그 전체를 떠서** style 속성을 통째로 읽는 방식으로 짰다.

**═══ ⑩ ⚠️ 겪은 함정 2가지 ═══**
(a) 🔴 **dev 서버가 뜬 채로 `npm run build` 를 돌려 `.next` 가 덮어써졌다** — 그 뒤 관리자
회귀 측정이 전부 `/admin/login` 에 머물고 서체가 **Times New Roman**(CSS 미로딩)으로 나와
"회귀 발생"으로 오판할 뻔했다. 41차 기록의 함정 b 와 같다. **빌드 전에 dev 서버를 죽이고,
측정 전에 `.next` 를 지우고 새로 띄울 것.**
(b) 🔴 **`kill $(cat pid)` 뒤에 같은 줄에서 새 프로세스의 PID 를 덮어쓰면 옛 프로세스가
안 죽는다** — `&` 백그라운드가 kill 보다 먼저 평가돼 PID 파일이 갱신되기 때문이다.
**죽이고 → 포트가 빈 것을 확인하고 → 새로 띄운 뒤 PID 를 기록할 것.**
...
**22차=52차 · 23차=53차 · 24차=54차** 세션(🔴 **53·54차는 같은 세션이다** — 조사를 먼저 하고
이어서 구현했다) — 29차는
...
- ~~**25차: 화주포털 화면 ② (발주 흐름)**~~ — **55차 세션에서 완료**(상세는 위 55차 기록).
  ...
  이후 세션이 알아야 할 것만 요약:
  - 🔴 **상하차조건 8종을 시안 6종으로 합치지 말 것** — `호이스트`≠`크레인`(장비 수배 여부가
    갈린다) · `협의필요`≠`기타`(라벨을 바꾸면 문자열 완전일치 매칭에서 조용히 가산이 빠진다) ·
    `컨베이어` 유지
  ...
  - 🔴 **상차 하한은 `max(오늘, LEGAL_EFFECTIVE_DATE)`** 이고 막는 곳이 셋이다(input · 칩 ·
    제출 직전). ⚠️ 9/7 전까지 「오늘」「내일」 칩이 비활성인 것은 **의도된 동작**이다
...
```
   ...
   2 발주 시작일     max(오늘, LEGAL_EFFECTIVE_DATE) = 2026-09-07     25차 ✅
