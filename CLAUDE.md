...
⚠️ **Go-Live 차단은 「랜딩 이미지」 1건**이고 디자인팀 결과물에 달려 있다.
...
```
  ...
  ——   담당자 정보 (차수 없음)   ← 다음. 29차에서 뺐다(계정 화면이라 파일이 안 겹친다)
  30차  내부 정합 ① 견적·오더        🟢 전제 해소 — 월정산 **(A) 확정**(2026-08-29)
  31차  내부 정합 ② 배차·정산
  32차  내부 정합 ③ 화주·차주
```
🔴 **다음은 차수 없는 소수정(담당자 정보 `customer/profile`)이고, 그 뒤가 30~32차다.**
