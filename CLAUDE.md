...
`components/pv2/Pv2DispatchCalendar.tsx`). 커밋 7개. 🟢 **DB 변경 0** ·
...
```
  ...
  (c) 문제발생·사진·인수증·혼적·추가비   시안에 0건 → 🔴 **그대로 뒀다** (원칙 42번)
...
🔴 **날짜 클릭 상세는 34차 기능이라 지우지 않았다.** ⚠️ 시안은 클릭 위치에 뜨는
팝오버이지만 **표 아래 카드**로 뒀다 — 팝오버는 칸마다 위치를 계산해야 하고 모바일에서
화면을 덮는다. ⚠️ 마지막 줄을 빈 칸으로 채워 7의 배수로 맞춘다(안 하면 `:nth-child(7n)`
세로선 규칙이 어긋난다).
