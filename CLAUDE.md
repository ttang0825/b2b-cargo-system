...
**작성일: 2026-07-23** (최종 갱신: 2026-07-29, Claude Code 세션에서 직접 갱신 — 3차
...
- **혼적 옵션(4차 세션)**: 동의·할인조건은 견적 단계에서 1회만 수집하고,
  ...
  지원. `lib/settlementCalc.ts`의 `applyMixedDiscount()`가 유일한 할인
  계산 로직 — `loading_type==='mixable' && mixed_executed`일 때만 할인이
  적용되고, 독차로 실제 운행됐으면(`mixed_executed=false`) 할인 미적용.
  배차 상세에서는 원본 청구운임 입력값은 그대로 두고(협의된 기준가), 마진
  계산·정산(invoices) 자동/수동 등록에만 할인 반영 후 금액을 사용 — 두
  값이 다를 때는 "할인 적용 후 화주청구금액"을 별도로 표시해 구분함
...
- **혼적 옵션(4차 세션)**: 동의·할인조건을 견적 단계에서 1회만 수집(적재구분
  ...
  없이 항상 수동 입력. `lib/settlementCalc.ts`의 `applyMixedDiscount()`가
  계산 로직 — 배차 상세의 마진 계산과 정산(invoices) 자동/수동 등록 시에만
  할인이 반영된 금액을 사용하고, 원본 청구운임 입력값 자체는 그대로 유지.
  견적서 출력(PDF)에는 혼적가능+화주동의 건에 "혼적 조건부 할인: OO%
  (실제 혼적 시 적용)" 문구 표시, 정산관리 목록·상세에는 배차의
