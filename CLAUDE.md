...
**작성일: 2026-07-23** (최종 갱신: 2026-08-18, Claude Code 세션에서 직접 갱신 — 33차
...
- **9차(다음): 동의 절차 구현. 착수 전 3가지를 사용자와 먼저 확정할 것** — (a) 동의문구
...
- 🔴 **5차 완료로 확정된 운영상 파급 — 별도 세션 과제**: 기준표를 공개하지 않고 추가비
  근거를 "견적 시 개별 고지"로 옮겼으므로(약관 제10조 3항·제11조 2항·제16조),
  **견적서에 무료 대기시간과 대기료 단가가 실제로 표기되어야 약관이 성립함.**
  29차 조사 결과 현재 견적서 PDF(`app/admin/quotes/[id]/print`,
  `app/customer/quotes/[id]/print`)에는 "실제 상하차 조건 및 대기시간에 따라 금액이
  변동될 수 있습니다"라는 **모호한 안내만 있고 구체 기준이 없음**. 다행히 **값은 이미
  운임기준표(`rate_vehicle_extra_fees`)에 톤수별로 있으므로 견적서에 찍기만 하면 됨.**
  ⚠️ **회차비는 사전 단가 자체가 없으므로 견적서에도 넣지 말 것**(약관 제11조 2항이
  회차비를 사전 안내 대상에서 뺀 것과 같은 이유 — 29차 확정 (B)안)
