...
세션: "화주포털 화면 ① — 셸·토큰·아이콘·홈(24차)" — 신규 파일 7개
(`components/PortalIcon.tsx` + `public/portal/` 에셋 5종 + `public/` 폴더 자체).
커밋 5개. 🔴 **DB 변경 없음.** 🟢 **관리자 회귀 0 — 4화면 × 11지표가 before/after 완전 동일.**
...
지시서가 제시한 두 후보(`위캐리운송(주)` / `(주)디자인에그`) **어느 쪽도 아닌 결합 형태**다.
확정 표기 `(주)디자인에그 (위캐리운송)` 과 **구분자(— vs 괄호)와 띄어쓰기가 둘 다 다르다.**
🔴 **금지 8대로 고치지 않았다.** 견적서·약관·처리방침·세금계산서·SMS 에 사업자 표기가 흩어져
있어 **별도 차수**다. ⚠️ 시안 견적서 공급자는 `위캐리운송(주)` 로 찍혀 있다(25차가 만난다).
...
관찰 대상 — **한 번에 하나씩**) · **anon ③(GRANT 회수)** · **화주포털 화면**(23·24차).
...
19차 anon RLS 정리 ① → 21차 anon RLS 정리 ②+④ → **22차 운임 매트릭스 보정 + 차급 11종까지 완료**.
🔴 **차수 순서가 한 번 바뀌었다** —
...
🔴 **그래서 화주포털 화면 개편이 23·24차로 또 밀렸다** — 21·22 → 22·23 → **23·24**로
**세 번 밀린 것이며 네 번째 예고**다. 실행 순서대로 번호를 붙이는 원칙 때문이니,
예고 번호가 아니라 **실제 실행 순서**를 기준으로 볼 것.
🔴 **다음은 23차(화주포털 화면 ①)**이고, **③(anon GRANT 회수) · 상차 시각 지정 가산 ·
...
- ~~**23차: 화주포털 화면 ①**~~ → 🔴 **23차는 사전조사가 됐다**(53차 세션, 위 참고).
  **화면은 24·25차로 밀렸다 — 다섯 번째 밀림이다**(21·22 → 22·23 → 23·24 → **24·25**).
  실행 순서대로 번호를 붙이는 원칙 때문이며, **예고 번호가 아니라 실제 실행 순서가 기준**이다.
- 🔴 **다음 지시서는 24차(화주포털 화면 ①)다** — 셸·사이드바 248px·모바일 탭바 5개·
  바텀시트·아이콘 18개·디자인 토큰(`.portal-v2` 스코프) + **홈**. 그 다음이 **25차(화면 ②)** —
  발주 요청 · 견적 확인(목록 + **상세 신설**) · 배차·운송 조회 · 정산 · 월별 통계 ·
  배송지·화물 관리 · 담당자 정보 · 견적서 print 2종.
  🔴 **9/7 을 기다리지 않고 바로 이어간다**(2026-08-26 확정) — 화주포털은 로그인한 화주만
  보므로 공개 리스크와 무관하다.
  🔴 **착수 전에 조사 보고서(`화주포털_개편_사전조사_23차.md`)를 먼저 읽을 것** — 지시서
  전제가 5건 틀렸고(홈은 이미 있다 · 부가세 위계는 안 뒤집힌다 등) 사용자 결정 항목이 11개다.
- 🟢 **20차는 전부 끝났다**(3-4는 47차, 3-1·3-2·3-3은 48차. 운영 DB 반영까지 완료).
  ⚠️ **24·25차 화면이 그 위에 올라간다** — 배송지 담당자·프리셋·`도크`를 쓰는 화면이
  아직 없는 것은 **의도된 상태**다. 🔴 **`customer_locations` 의 20차 추가 3컬럼과
  `location_name`·`notes` 는 12행 전부 비어 있다**(53차 실측) — 25차가 처음 쓰게 된다.
  🟢 **51차가 `customer_locations`·`customer_presets` 를 화주 롤로 실측해 뒀다** — 자기
  회사 것만 읽고 쓸 수 있다. 23차에서 "저장이 안 된다"가 나오면 정책이 아니라 화면 쪽이다.
