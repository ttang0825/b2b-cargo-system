...
**작성일: 2026-07-23** (최종 갱신: 2026-08-26, Claude Code 세션에서 직접 갱신 — 54차
...
⚠️ **PR #102 에서 사용자 확답을 못 받았다** — 25차 착수 전에 한 번 더 물을 것.
...
- ~~**24차: 화주포털 화면 ① (셸·토큰·아이콘·홈)**~~ — **54차 세션에서 완료**(PR #102 merge
  ...
  - ⚠️ **자동검증 27개가 전부 통과했는데도 시안과 다른 것을 하나도 못 잡았다** —
    ...
    대조하는 검증을 넣을 것**
- 🔴 **다음 지시서는 25차(화주포털 화면 ②)다** — 발주 요청(카테고리 6개 복원 · 상하차 8종 ·
  상차 `minDateTime` · 배송지 담당자) · 견적 확인(목록 + **상세 신설**) · 배차·운송 조회
  (3단계 알약 + 문제발생 배지) · 정산 · 월별 통계(+캘린더) · 배송지·화물 프리셋 ·
  담당자 정보 · 견적서 print 2종(부가세 행).
  🔴 **9/7 을 기다리지 않고 바로 이어간다**(2026-08-26 확정) — 화주포털은 로그인한 화주만
  보므로 공개 리스크와 무관하다.
  🔴 **착수 전에 조사 보고서를 먼저 읽을 것** — 사용자 결정 항목이 11개 남아 있다.
  🔴 **25차가 함께 처리해야 하는 것 3개**: 홈의 배차 배지 6종 → 3단계(54차 ⑦ — **아직
  사용자 확답 없음, 먼저 물을 것**) · `/customer/request` 상차 `minDateTime` 누락(과거 날짜로
  발주가 된다, 원칙 6 위반) · 견적서 PDF ↔ 엑셀 부가세 표기 불일치(31차가 금지한 상태).
- 🟢 **20차는 전부 끝났다**(3-4는 47차, 3-1·3-2·3-3은 48차. 운영 DB 반영까지 완료).
  ⚠️ **25차 화면이 그 위에 올라간다** — 배송지 담당자·프리셋·`도크`를 쓰는 화면이
  아직 없는 것은 **의도된 상태**다(24차는 셸·홈만 했다). 🔴 **`customer_locations` 의 20차
  추가 3컬럼과 `location_name`·`notes` 는 12행 전부 비어 있다**(53차 실측) — 25차가 처음 쓴다.
