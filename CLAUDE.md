...
**작성일: 2026-07-23** (최종 갱신: 2026-08-25, Claude Code 세션에서 직접 갱신 — 45차
...
9차=34차, 10차=35차, 11차=37차, 12차=39차, 13차=40차, 14차=43차, 15차=44차, **16차=45차** 세션 — 29차는
...
붙인 결과가 아래이며, **16차까지 전부 끝났다** — 🔴 **다음은 17차(차급 9종 확장)이며,
그다음이 18차(가산기준 조정)다**:
...
- 🔴 **가장 먼저 — 다음 지시서는 17차(차급 9종 확장)다.** 그다음이 18차(가산기준 조정 —
  파손주의 · 장척/중량 · 상차 시각 지정)다.
  🔴 **17차에서 손볼 차급 배열은 6곳이고, 공개 화면까지 세면 8곳이다**(45차에 grep 전수 확인):
  `lib/constants.ts:24`(정본) · `admin/quotes/page.tsx` · `admin/rates/page.tsx` ·
  `admin/drivers/page.tsx` · `admin/drivers/[id]/page.tsx` · `admin/quotes/[id]/page.tsx`(인라인)
  — 여기에 **`/vehicles`의 `VEHICLE_SPECS`·`START_PRICES` 2개**를 더해야 한다.
  🔴 **뒤 2개는 9종으로 늘리면 안 되는 곳이다** — 공개 문구 기준이 "1톤부터 5톤 이상까지"이고
  `"25톤"`은 금지다(32차·12차 확정). 이것이 `VEHICLE_TYPES_ALL` / `VEHICLE_TYPES_PUBLIC`
  분리가 필요한 이유다. ⚠️ **행 번호는 바뀌므로 grep으로 다시 찾을 것.**
  ⚠️ 17차는 `rate_distance_tiers`에 **45행(15구간 × 3차급) INSERT**와
  `rate_vehicle_extra_fees` **3행 INSERT**가 따라온다. 🔴 **드롭다운 배열에만 차급을 추가하고
  INSERT를 빠뜨리면** 그 차급 견적이 조용히 실패하는데, 15차가 넣어둔
  "선택하신 톤수에 해당하는 운임기준이 없습니다" 문구가 그때 원인을 알려준다.
- ⚠️ **사용자가 배포본에서 확인할 것(16차 잔여)**: `/vehicles` 시작가가 **48,000원부터**로
  보이는지, `/admin/quotes`에서 거리 `10.4` · 톤수 `1톤` → **60,000원**, 톤수 `5톤` ·
  대기시간 `90` → 대기료 **50,000원**. 이 환경에서는 자격증명·프록시 제약으로 확인할 수 없다.
