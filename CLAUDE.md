...
**작성일: 2026-07-23** (최종 갱신: 2026-08-26, Claude Code 세션에서 직접 갱신 — 47차
...
더하는 것이 다음 차수 후보다. ⚠️ 발주요청을 지워도 `consents` 행은 남는다(FK 없음, 14차 의도).
...
15차 거리 구간 매칭 버그 수정 →
16차 운임 기준 교체 →
17차 차급 9종 확장 → **20차 3-4(제3자 제공 동의)까지 완료**. 🔴 **차수 순서가 한 번 바뀌었다** —
...
9차=34차, 10차=35차, 11차=37차, 12차=39차, 13차=40차, 14차=43차, 15차=44차, 16차=45차, 17차=46차, **20차 3-4=47차** 세션 — 29차는
...
붙인 결과가 아래이며, **17차 + 20차 3-4까지 끝났다** — 🔴 **다음은 18차(약관 동의)이며
Go-Live 차단 항목이다**(시행일 2026-09-07). 그 다음이 19차(가산기준 — 17차 실측으로 범위가
늘었다)이고, **20차 나머지(3-1·3-2·3-3)도 남아 있다.** 아래 5번 첫 항목들을 볼 것:
...
- ~~**20차 3-4: `/customer/request` 제3자 제공 동의**~~ — **47차 세션에서 완료**(PR #96,
  ...
  - ⚠️ **3-1·3-2·3-3은 안 했다**(아래 "다음" 항목 참고)
...
- 🔴 **그 다음이 20차 나머지 3건이다**(3-4만 47차에 먼저 처리했다). 전부 **DB 변경이 따라오며,
  이제 마이그레이션 파일을 만들면 세션이 `dry-run` → `apply`로 직접 돌릴 수 있다.**
  ① **3-1 `customer_locations` 컬럼 추가**(`name`·`contact_name`·`contact_phone`·`note`,
  전부 nullable. `address_detail`은 **이미 있는지 먼저 확인**할 것) ② **3-2 `customer_presets`
  신설**(`preset_type` = `cargo`/`request`, `payload` jsonb. 🔴 **테이블을 둘로 나누지 말 것,
  payload를 정규화 컬럼으로 펼치지 말 것** — 프리셋은 폼에 채워넣기 용도이지 집계 대상이 아니다)
  ③ **3-3 `rate_surcharges`에 `도크` 1행 + `lib/loadingMethods.ts` 8종**(🔴 행이 없으면
  `/admin/rates`에서 어떤 값을 넣어도 `도크` 건에 가격이 안 붙는다 — 견적 계산이 문자열
  완전일치로 매칭하고 못 찾으면 **예외도 경고도 없이 가산만 빠진다**. `도크`는 `지게차` 다음).
  ⚠️ **21·22차가 화면 개편이다** — 20차 나머지는 그 화면들이 저장할 곳을 만드는 작업이다.
