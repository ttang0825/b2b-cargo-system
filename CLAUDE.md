...
> 🔴 이 문서(`CLAUDE.md`)는 **252,456자**입니다(2026-09-16 38차 접수 알림 #160 직후) —
> 컨텍스트 예산 판단에 쓰는 값입니다. ⚠️ 그 직전에는 246,209자, 그 앞에는 241,568자였습니다.
> ⚠️ **`current.md` 가 또 상한(80,000자)을 넘어 `history-14.md` 로 갈랐습니다**(#151~#154 5건을
> 옮김 · **md5 대조로 한 글자도 안 지웠음을 확인**했습니다). 50,543자.
...
| `docs/history/current.md` | 운송오더 3건 2026-09-15  →  38차 접수 알림 #160 2026-09-16   (6건) | 50,543자 |
...
```
  ...
  ——   화주포털 B장 (웹 푸시)   🔴 **하기 전에 정할 것** — 배차 상태 변경은 **이미 화주에게
        SMS 가 나간다**(23차 · 원칙 53번). 푸시까지 붙이면 **같은 일로 두 번** 알린다.
        🟢 서버쪽 신원 확인 경로는 이미 있다(`Bearer → auth.getUser() → company_id`)
...
```
  ...
  ⛔ B장 선행       **화주포털 웹 푸시는 배차 SMS 와 겹친다**(23차) — 푸시로 옮길지, 둘 다
                  둘지, 견적·정산만 푸시할지를 **지시서가 먼저 정해야 한다**
