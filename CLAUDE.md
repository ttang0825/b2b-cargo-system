...
**작성일: 2026-07-23** (최종 갱신: 2026-07-30, Claude Code 세션에서 직접 갱신 — 10차
세션: "화주포털 표시 개선 세션 A" — admin에는 이미 있었는데 화주포털에는 빠져있던
표시 5건을 보강함(혼적가능 배지를 포털 견적확인/배차·운송조회/정산확인 3개
...
별도/포함 병기) + CLAUDE.md에 새 원칙 42번(admin↔화주포털 데이터 동기화
확인 습관) 추가 완료·merge됨. DB 변경 없음(표시만 추가, 기존 컬럼만 조회).
9차
...
- **화주포털 표시 개선 세션 A(10차 세션)**: 9차 세션(0단계 사전점검)에서
  ...
  5건을 보강한 세션. 전부 표시 추가 위주라 DB 변경은 없음(기존 컬럼만
  추가로 조회).
  ...
  - **범위에서 제외**: 검색·정렬 개선(작업지시서 6번 항목)은 별도 세션
    B로 분리, 이번 세션에서 다루지 않음. 원칙 42번 점검 중 발견한 추가
    후보(정산방식 배지가 포털에 아직 없음)는 임의로 범위를 넓히지 않고
    사용자에게 별도 확인 요청함(처리 여부 미정 — 사용자 답변 대기 중)
...
- **정산관리(`invoices`) 상세 저장은 서버단 권한 체크가 없음** — 상태·금액·
  정산방식 변경이 anon 클라이언트로 직접 처리되어 role 무관하게 누구나
  가능함(의도된 설계로 보임). 향후 "정산 확정은 관리자만" 같은 요구사항이
  생기면 원칙 25번(화면단+서버단 이중체크)을 새로 적용해야 함(0단계 조사,
  9차 세션)
