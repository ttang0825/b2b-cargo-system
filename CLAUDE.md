...
줄이면 상담 품질이 떨어진다.
**(9) ⑤ 여백은 시안을 따르지 않고 판단했다** — 3단계만 남아 콘텐츠가 227px(1280 기준)로
줄었지만 **다른 섹션과 같은 88/56px 패딩을 그대로 뒀다.** 진행선이 가로 전폭을 지나가서
띠가 비어 보이지 않고, 여기만 패딩을 줄이면 섹션 리듬이 어긋난다(스크린샷으로 확인).
...
- ~~**12차: 정합성·상수·진입경로·이미지 구조**~~ — **39차 세션에서 완료**(상세는 위 39차
  ...
  - **`/customer/login`에 공개 화면 헤더가 붙는다**(`CustomerPortalShell`의 PUBLIC_PATHS
    분기). noindex는 영향 없음. 🔴 헤더를 다시 떼면 폼 하단 "← 홈으로"를 되살릴 것.
    ⚠️ 같은 분기를 타는 `/customer/support-verify`에도 함께 붙는다
  - **`lib/insuranceInfo.ts`가 있지만 참조하는 UI는 0건**(의도된 것, 13차 범위).
    🔴 **아직 미가입이라 `INSURANCE_ENABLED = false`** — 켜기 전에 화면에 노출하면
    허위·과장 광고다. 별도 플래그를 만들지 말고 이 하나로 두 곳을 제어할 것
...
- ~~**13차: 랜딩 섹션 신설 — 운송관리·안전책임**~~ — **40차 세션에서 완료**(상세는 위 40차
  ...
  - ⚠️ **360px 헤드라인은 3줄이다**(37차부터의 상태). 2줄로 만들려면 28px → 23px 이하로
    줄여야 해서 손대지 않았음 — 줄이려면 시안 확정값을 바꾸는 판단이 필요하다
