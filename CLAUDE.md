...
**작성일: 2026-07-23** (최종 갱신: 2026-08-26, Claude Code 세션에서 직접 갱신 — 48차
...
15차 거리 구간 매칭 버그 수정 →
16차 운임 기준 교체 →
17차 차급 9종 확장 → **20차 전체(3-1·3-2·3-3·3-4)까지 완료**. 🔴 **차수 순서가 한 번 바뀌었다** —
...
9차=34차, 10차=35차, 11차=37차, 12차=39차, 13차=40차, 14차=43차, 15차=44차, 16차=45차, 17차=46차, 20차 3-4=47차, **20차 3-1·3-2·3-3=48차** 세션 — 29차는
...
붙인 결과가 아래이며, **17차 + 20차 전체까지 끝났다** — 🔴 **다음은 18차(약관 동의)이며
Go-Live 차단 항목이다**(시행일 2026-09-07). ⚠️ **지시서가 필요하다** — 화면 법적 문구를
고쳐야 성립하는 작업이라 문안을 임의로 지어낼 수 없다. 그 다음이 19차(가산기준 — 17차
실측으로 범위가 늘었다)이고, 그 뒤가 **21·22차 화면 개편**이다. 아래 5번 첫 항목들을 볼 것:
...
- 🔴 **가장 먼저 — 다음 지시서는 18차(약관 동의)다. Go-Live 차단 항목이다.**
  시행일 **2026-09-07**이 확정되면서 20차 지시서가 이 순서를 못박았다(가산기준은 19차로 밀림).
  🔴 **14차가 남긴 제약을 먼저 볼 것** — 지금 `terms` 동의를 받는 화면이 **하나도 없고**
  `TERMS_VERSION`이 계속 미사용이다. `/quote`·`/apply` 문구가 "개인정보 수집·이용"으로만
  끝나고 **이용약관을 언급하지 않기 때문**이다. 🔴 **`CONSENT_TYPES_BY_SOURCE`에 `"terms"`를
  먼저 넣지 말 것** — 받지 않은 동의를 기록하면 그 기록이 거짓이 되어 입증 자료로 쓸 수 없다.
  **화면 문구를 먼저 고쳐야** 하며 그것이 18차의 실체다.
  🟢 **20차 3-4가 본보기다** — 문구는 `lib/legalInfo.ts` 상수로, 저장은 `recordConsents`로,
  서버 게이트는 `agreed !== true`로. 구조를 새로 만들 것이 없다.
- 🟢 **20차는 전부 끝났다**(3-4는 47차, 3-1·3-2·3-3은 48차. 운영 DB 반영까지 완료).
  ⚠️ **21·22차 화면 개편이 그 위에 올라간다** — 배송지 담당자·프리셋·`도크`를 쓰는 화면이
  아직 없는 것은 **의도된 상태**다.
- 🔴 **그 다음이 19차(가산기준 조정)다. 17차에 들어온 새 실측 때문에
