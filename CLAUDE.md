...
>
> 저장소 루트에 **`HANDOFF.md`** 가 있습니다(27차). 두 문서는 **역할이 다릅니다.**
>
> | 문서 | 성격 | 언제 |
> |---|---|---|
...
>
> 🔴 **`HANDOFF.md` 는 `CLAUDE.md` 의 대체가 아니라 입구입니다.** 이 문서가 약 63만
> 자라 통째로 읽을 크기가 아니어서 둔 것이고, **이 문서를 요약해서 줄이지 마십시오** —
...
```
  ...
  32차  내부 정합 ① 견적·오더        ← 다음 · 🟢 전제 해소 — 월정산 **(A) 확정**(2026-08-29)
  33차  내부 정합 ② 배차·정산
  34차  내부 정합 ③ 화주·차주
  ——   admin 공개문의 배지 · 보험 문구 정합 · 접수 안내 문자
```
🔴 **내부 정합이 30~32차에서 32~34차로 밀렸다** — 30차가 랜딩 교체가 되고 31차가 폼
3화면이 되면서다. **예고 번호가 아니라 실제 실행 순서를 기준으로 볼 것**(여덟 번째 밀림).
🔴 **다음은 32차(내부 정합 ① 견적·오더)다.** 🟢 **담당자 정보(`customer/profile`)는
끝났다** — 남은 「차수 없는 소수정」은 admin 공개문의 배지 하나뿐이다.
...
```
  ...
  🟢 프리렌더 기준선     **43** (44 에서 `/about` 하나가 빠졌다. `/vehicles` 는 64차 ③ 이후
                      동적 라우트라 원래 그 목록에 없었다)
```
...
- ⏸️ **가산기준 조정 — 22차에 대부분 끝났고 두 가지가 남았다.**
  ...
  - 🔴 **5톤급 `base_fare` 재검토** — 실거래 42건에서 마진 중앙값 **13.6%**(목표 17% 미달).
    ⚠️ **표본 8건이라 관찰 대상**이다. 18톤(완료 8건)처럼 근거가 서면 그때 조정할 것.
...
```
  ...
  ② CLAUDE.md    이 문서. 세션별 축적 기록 — 그 결정을 왜 그렇게 했는지 되짚을 때
       ...
       ├ 5장  다음 예정 작업 (로드맵·차수)
       └ 7장  자주 막히는 지점
  ③ 해당 차수의 세션 기록  이 문서 최상단부터 시간 역순으로 쌓여 있다
```
