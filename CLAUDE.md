...
> 🔴 이 문서(`CLAUDE.md`)는 **239,842자**입니다(2026-09-16 24시콜 샘플 실측 직후) —
> 컨텍스트 예산 판단에 쓰는 값입니다. ⚠️ 그 직전에는 236,835자, 그 앞에는 232,813자였습니다.
> ⚠️ **`current.md` 가 상한(80,000자)을 넘어 `history-13.md` 로 갈랐습니다**(#147~#150 5건을 옮김 ·
> 공백 제외 대조로 한 글자도 안 지웠음을 확인 — 53,074자가 그대로 보존됐습니다). 49,352자.
...
| `docs/history/current.md` | 36차 PR 2 #151 2026-09-15  →  24시콜 v4 차수 변경 + 샘플 실측 2026-09-16   (9건) | 63,555자 |
...
```
  ...
  ——   운임기준표 v12 C장 · 소계 반올림 + 견적서 가산 한 줄 (차수 없음)  ✅ 완료 (PR #158 merge
        ...
        **merge 후 실물 확인 대상**이다 ·
        ⏳ **답 대기 1건** — 조건이 많으면 라벨이 길어진다(가로 넘침은 없다). **몇 개까지 적고
        「외 N건」으로 줄일지** 정하면 한 줄로 바꾼다)
  ...
  37차  24시콜 가져오기 ⓐ — 배차 정보 채우기   ← **다음**   🟢 **샘플 받음·분석 완료**
        🔴 **2026-09-16 에 바뀌었다** — 입력 방식이 OCR·캡쳐에서 **붙여넣기 + 라벨 파싱**으로
  ——   24시콜 ⓑ 견적 폼 → ⓒ 정산 엑셀
...
```
  ...
  ⚠️ 안 한 것       **print 2종 실물 확인**(목이 불완전해 크래시 — `origin/main` 도 똑같이 크래시해서
                  ...
                  `물품특성`·`상하차방식` 정액 전환(실측은 끝났다) · **라벨 「외 N건」 답 대기**
...
```
  ...
  🟢 부가세 미결이 닫혔다   **합계 80,000 이 공급가액이다.** 화면의 두 값이 그것을 증명한다 —
        `합계 80,000 × 1.1 = 88,000` · `88,000 − 산재 350 = 87,650`(= 화면의 「송금할운임
        (부가세포함)」과 **정확히 일치**). 🔴 **따라서 운송료 65,000 과 수수료 15,000 은
        둘 다 부가세 별도다** · 🔴 **35차는 `brokerage_fee` 를 「부가세 포함가 기입」으로
        확정했으므로 그대로 넣으면 마진이 10% 적게 잡힌다** — ⓐ 는 **15,000 × 1.1 =
        16,500** 을 넣거나 그 칸의 기준을 바꿔야 한다. **ⓐ 지시서가 정할 것**
  ...
  🔴 산재 기준액이 다르다   24시콜은 **합계(80,000) 기준 + 10원 절사**다 —
        `floor(80,000 × (1−49.9%)) = 40,080` → `floor(40,080 × 1.76%/2) = 352` → **350**.
        🔴 **우리 `calcSettlement()` 는 `driverBaseFare`(65,000) 기준이라 286 이 나온다.**
        같은 건의 산재가 **350 vs 286** 으로 갈린다 — ⓐ 지시서가 「가져온 값을 그대로 둘지
        우리 식으로 다시 계산할지」를 정해야 한다. 🔴 **말없이 재계산하면 24시콜 정산서와
        대조가 어긋난다**(v12 C장 반올림과 같은 자리)
...
🔴 **다음은 36차 PR 2(C·D·E장 · DB 0)이고, 그 뒤가 37차(CRM 항목 신설) → 24시콜 가져오기
ⓐ→ⓑ→ⓒ 세 차수다.** ⚠️ **거래조건은 37차가 아니라 36차가 가져갔다** — 37차에 남은 것은
CRM 항목 신설(업종·잠재등급·주요 노선 등)과 차주다.
