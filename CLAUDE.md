...
**작성일: 2026-07-23** (최종 갱신: 2026-08-29, Claude Code 세션에서 직접 갱신 — 60차
세션: "화주포털 조회 흐름(29차)" — 신규 파일 2개(`lib/dispatchStage.ts` ·
`components/pv2/Pv2DispatchCalendar.tsx`). 커밋 10개(본작업 7 + **실사용 리뷰 3**). 🟢 **DB 변경 0** ·
...
```
  ...
  포털 2화면       화주 말로 바꾸되 **축을 쪼갠다** — 29차 범위
...
⚠️ **🔴 화주포털에 아직 담당자 말이 남아 있는 곳이 두 곳 있다 — 29차가 볼 것.**
...
**22차=52차 · 23차=53차 · 24차=54차 · 25차=55차 · 26차=56차 · 27차=57차** 세션(🔴 **53·54차는 같은 세션이다** —
조사를 먼저 하고 이어서 구현했다) — 29차는
...
붙인 결과가 아래이며, **17차 · 20차 전체 · 18차 · 19차 · 21차 · 22차 · 23차 · 24차 · 25차까지 끝났다** —
...
```
🔴 **P1-1(포털 2화면 정산방식 라벨: 「운임 수금방식」 + 「청구」 별도 줄)은 29차 몫이다** —
포털 화면이라 시안이 필요해서 이번에 하지 않았다.
...
- ~~🔴 **다음 지시서는 29차(포털 조회 흐름)이고 시안이 있어야 착수한다.**~~ →
  ...
  **35컬럼**(마지막이
  ...
  다시 꺼내지 말 것. 그 다음이 **29차(조회 흐름)** — 배차·운송 조회(3단계 알약 +
  문제발생 배지 + 정산방식·혼적·사진 자리) · **홈 배차 배지 공용 매핑** · 정산 ·
  월별 통계(+캘린더) · 담당자 정보.
...
```
  ...
  3d 적재구분        독차/혼적가능 · 작은 원형 라디오                  25차 ✅
   ...
   8 정산방식        현행 4종 유지(2종으로 줄이지 않음)               29차
...
```
  ...
  금지한 상태였다). 🔴 **남은 하나**: 홈 배차 배지 6종 → 3단계(**29차**, 배차 화면과 같은
  공용 매핑을 한 번에 만들 것).
