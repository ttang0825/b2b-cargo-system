...
사용자가 Supabase에서 직접 실행 예정(이 세션 종료 시점 기준 아직 미실행 —
실행 전까지는 업로드가 실제로 동작하지 않음, 코드는 준비 완료 상태).
`files`/`profiles` 테이블은 이번에도 코드 전체 재확인 결과 앱 코드 어디서도
참조되지 않는 무관한 레거시 테이블로 재확인됨(손대지 않음). 16차
...
- **POD·인수증 도입(로드맵④, 17차 세션)**: 운송 완료 후 하차지 사진·인수증을
  ...
  배차·운송조회 목록 펼침 방식으로 열람. DB/버킷 설정은 사용자가 Supabase에서
  직접 실행 예정
...
> **진행 현황 갱신(16차 세션 기준)**: ①은 12차 세션(정산 마감·확정·잠금)에서
...
> 17차 세션에서 완료됨(DB/버킷 설정은 사용자 실행 예정, 아래 참고). ⑤~⑥은
> 여전히 미착수.
