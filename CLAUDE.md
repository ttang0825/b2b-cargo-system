...
커밋 17개(본작업 6 + **실사용 리뷰 6라운드 11**). **PR #105.**
...
🔴 **⑤ 제3자 제공 동의는 접이식이다.** 접힌 상태에 남는 것은 **제목 · 고지 문장(intro) ·
「자세히 보기」 · 체크박스** 넷이고, 펼치면 **제공받는 자·목적·항목·기간 4항목 · 확인 문장 ·
거부권 안내(refusal)** 가 나온다.
⚠️ **5라운드에는 거부권 안내도 밖에 뒀는데 6라운드에 사용자가 그것도 접으라고 확정했다**
(2026-08-28). 접는 것이지 없애는 것이 아니고, 토글이 체크박스 바로 위에 있어 한 번 눌러 볼 수
있다. 🔴 **"5라운드 기록에 밖에 두라고 되어 있다"며 되돌리지 말 것.**
🔴 **`intro` 는 접지 말 것** — 무엇을 왜 제공하는지까지 감추면 체크박스만 남아 **동의를 구하는
문장이 화면에서 사라진다.** ⚠️ 개인정보보호법 제15조 2항이 고지와 거부권 안내를 요구한다 —
접어도 화면에 있긴 하지만 **문구 자체가 변호사 검토 대상이다**(47차 이후 계속).
