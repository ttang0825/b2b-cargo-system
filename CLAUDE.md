...
확인 불가 — 사용자가 Vercel 대시보드에서 직접 확인 필요), `BODY_TYPES` 11종에
...
- 그 밖에 이번 범위에서 제외된 것: 데스크탑 섹션 패딩 목표치(96px) 미달, 견적서 PDF에
  대표번호 미표시(번호 확정 후), Vercel 함수 리전(서울 `icn1`) 확인 — **코드로 확인
  불가하므로 사용자가 Vercel 대시보드에서 직접 확인 필요**, SEO 일괄(도메인 확정 후:
