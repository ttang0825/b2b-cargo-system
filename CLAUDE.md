...
커밋 19개(본작업 6 + **실사용 리뷰 7라운드 13**). **PR #105.**
...
**═══ ③-5 🔴 리뷰 5라운드 — 화주 말 · 입금 계좌 · 블록 순서 · 동의 접이식 ═══**
...
```
⚠️ **답을 못 받은 것 3건** — ① (26차부터) *"자주 쓰는 화물을 불러오면 그 목록 이름이 어딘가
표시되면 좋겠다"* ② (25차부터) 무료 대기시간의 `"초과 시 30분당"` 단위도 20분으로 바꿀지
③ (4라운드) **선착불 견적서에도 입금 계좌를 그대로 둘지** — 화주가 차주에게 직접 지급하지만
주선수수료는 그 계좌로 청구될 수 있어 정책 판단이라 손대지 않고 물어뒀다.
...
- ~~**26차: 화주포털 시안 정합 (서체·아이콘·폼 컴포넌트)**~~ — **56차 세션에서 완료**
  ...
  - 🔴 **입력 크기는 세 벌이다**(`.pv2-input` / `-sm` / `-grid`) — 합치면 주소칸이 작아지고
    담당자칸이 커진다
  - ⚠️ **「내 요청 내역」을 지워서 반려 건을 볼 화면이 없다** — 자리를 정해야 한다(아래 참고)
