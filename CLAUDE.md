...
🔴 **PDF 2종에 같은 코드를 넣었고 `lib/quoteExcel.ts` 는 한 글자도 안 건드렸다**(이미 맞았다).
...
- **캘린더** → 🔴 **24차와 25차 사이에는 진입 경로가 없다. 의도된 공백이다.**
  25차에 월별 통계 화면 하단으로 들어간다(시안 렌더링에서 확인됨)
...
```
  ...
  28차  내부관리시스템 연동 조사     ← 다음 (조사 전용, 코드 0)
  29차  화주포털 조회 흐름           🔴 **시안이 있어야 착수한다**
  미정  내부 시스템 정합             🔴 **월정산 두 칸을 먼저 정해야 한다**(아래)
```
...
🔴 **다음 지시서는 28차(내부관리시스템 연동 조사)다 — 조사 전용, 코드·DB 변경 0.**
  ...
  🟢 **28차가 쓸 실측을 27차에 미리 떠뒀다** — `portal_order_requests` **35컬럼**(마지막이
