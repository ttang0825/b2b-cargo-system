...
> 🔴 이 문서(`CLAUDE.md`)는 **393,241자**입니다(2026-09-22 `history-21.md` 분리 직후) —
> 컨텍스트 예산 판단에 쓰는 값입니다. ⚠️ 그 직전에는 391,970자, 그 앞에는 384,020자였습니다.
> ⚠️ **`wc -m` 으로 세지 마십시오** — 이 환경은 locale 이 UTF-8 이 아니라 **바이트**가 나옵니다
> (한글 섞인 글은 실제의 약 1.9배). `python3` 의 `len()` 으로 세십시오.
> ⚠️ **`current.md` 는 지금 44,080자입니다**(상한 80,000자 · 2026-09-22 분리 직후) —
> 🟢 **상한까지 35,920자 여유가 있습니다.** 기록 한 건이 7,000~18,000자이니 **4건쯤** 들어갑니다.
...
| `docs/history/current.md` | 견적관리 전화응대 매뉴얼 #179 2026-09-18  →  기업고객 리워드 3차 #182 2026-09-21   (4건) | 44,080자 |
...
```
  ...
  ——   **다음 차수 미정**   ⚠️ 24시콜이 보류되면서 로드맵에 `← 다음` 이 없다.
        남은 후보는 **내부 정합 ② 화주·차주**(아래 · 뒤로 미뤄 둔 것)와
        **admin 공개문의 배지**(차수 없는 소수정) 둘뿐이고, 🔴 **둘 다 사용자가 고르기
        전에는 착수하지 말 것**
