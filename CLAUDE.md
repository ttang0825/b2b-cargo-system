...
> 🔴 이 문서(`CLAUDE.md`)는 **394,160자**입니다(2026-09-22 admin 공개문의 배지 닫음 직후) —
> 컨텍스트 예산 판단에 쓰는 값입니다. ⚠️ 그 직전에는 393,241자, 그 앞에는 391,970자였습니다.
> ⚠️ **`wc -m` 으로 세지 마십시오** — 이 환경은 locale 이 UTF-8 이 아니라 **바이트**가 나옵니다
> (한글 섞인 글은 실제의 약 1.9배). `python3` 의 `len()` 으로 세십시오.
> ⚠️ **`current.md` 는 지금 46,509자입니다**(상한 80,000자 · 2026-09-22 실측) —
> 🟢 **상한까지 33,491자 여유가 있습니다.** 기록 한 건이 7,000~18,000자이니 **3~4건쯤** 들어갑니다.
...
| `docs/history/current.md` | 견적관리 전화응대 매뉴얼 #179 2026-09-18  →  admin 공개문의 배지 닫음 2026-09-22   (5건) | 46,509자 |
...
```
  ...
  ⚠️ 안 한 것       운임 할인 적용 · 상품권 지급 · 소멸(`expiry` 유형 없음) · 「처리 필요」의
                  지급 버튼 · 미적립 건 목록 ·
...
```
  ...
  ⚠️ 안 한 것       이용가이드에 **적립금 항목을 안 넣었다**(가이드 `scope` 는 화주별 조건을
                  ...
                  운임 할인 적용 · 상품권 지급 · 소멸(`expiry` 유형 없음) · 지급 버튼 ·
                  미적립 건 목록 · 포털 적립금 **사용** 흐름(전부 3차)
```
...
```
  ...
  ⚠️ 안 한 것       🟢 **소급 적립은 만들었다**(위) — 「후보 0건이라 안 만든다」는 낡았다.
                  ...
                  운임 할인 적용 · 상품권 지급 · 소멸(3차 · 원장에 `expiry` 유형이 없다) ·
                  「처리 필요」의 지급 버튼(3차 — 1차는 **목록만**) ·
                  **미적립 건 목록**(적립 실패를 볼 자리가 정산 상세뿐이라 **다음에 할 일**)
```
