...
> 🔴 이 문서(`CLAUDE.md`)는 **300,732자**입니다(2026-09-17 히어로·헤더 로고 선명도 #169 직후) —
> 컨텍스트 예산 판단에 쓰는 값입니다. ⚠️ 그 직전에는 294,700자, 그 앞에는 289,588자였습니다.
> ⚠️ **`current.md` 를 `history-16.md` 로 갈랐습니다**(2026-09-17 · 24시콜 v4 · 37차 ⓐ #159 ·
> 38차 #160 · 포털 B장 #161 · 알림 4건 #162 **5건**을 옮김 · **md5 대조로 한 글자도 안 지웠음을
> 확인**했습니다 — `e636e43c4f0ed5cea14590f07915c430`). 지금은 **50,932자**다.
...
| `docs/history/current.md` | 견적번호 중복 #163 2026-09-16  →  히어로·헤더 로고 선명도 #169 2026-09-17   (7건) | 50,932자 |
...
```
  ...
  39차  견적문의 폼 정합 + 운송시간 자동선택   ✅ 완료 (PR #165 merge `8c8a1c5` — 🟢 **DB 0** ·
        ...
        **세 화면**이 쓴다. ⚠️ **18:00~19:59 상차 건이 0원 → +10,000원으로 오른다**(사용자 확정) ·
        🔴 **주말이 시간대보다 먼저다**(바꾸면 주말 새벽이 또 오른다 — **먼저 물을 것**) ·
  ...
  ——   랜딩 히어로 사진·헤더 로고 선명도 (차수 없음)  ✅ 완료 (PR #169 merge `f656abb` —
        ...
        ⚠️ **`cta-bg.jpg` 도 같은 상황이고 안 고쳤다** · ⚠️ 커밋 4는 Preview 확인 없이 merge)
...
```
  ...
  ⚠️ 안 한 것       **`cta-bg.jpg` 도 같은 상황이다**(1600px 화면에서 약 0.55배 축소) —
                  같은 방식이 먹힐 자리이고 안 고쳤다 ·
                  **521~1120px 구간의 로고(30px·36px)도 한글이 18~21px 이다** — 컴팩트 판을
                  그 구간까지 넓힐지는 **사용자가 정한다**(보고했고 답은 아직 없다) ·
...
```
  ...
  🔴 동까지만은 안 된다 다음 우편번호 팝업은 **건물·지번 단위만** 준다 — 🟢 **사용자 실물
                  ...
                  **우편번호가 항목마다 다르다** 16480/16490. 우편번호가 건물·구역 단위라
                  ...
                  「안 고르는 길」로 못박았고** 별도 차수 크기다. **사용자가 실물 확인 후 정한다**
...
```
  ...
  ⚠️ 금액이 오른다  **18:00~19:59 상차 건의 견적이 0원 → +10,000원**이 된다(사용자 확정
                  ...
                  🔴 **주말이 시간대보다 먼저다**(기존 동작 유지) — 바꾸면 주말 새벽이
                  +10,000 → +20,000 으로 **또 오른다. 먼저 물을 것** ·
  ...
  ⚠️ 사용자 몫     **Preview 미확인**(리뷰 반영 직후 merge 지시) — 빨간 `*` 톤 · 포털 표시 ·
                  ...
                  알렸고 답은 없었다) · ⚠️ **주말 vs 새벽 우선순위**는 열려 있다
```
