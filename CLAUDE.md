...
실행 확인 완료(작업지시서 원안의 신규 컬럼 중 2개는 기존 컬럼 재사용으로 판단,
아래 5번 섹션 참고))
...
- **차주 수금/지급 운임 표시 구조**: `dispatches`에 `driver_base_fare`(차주
  ...
  `industrial_insurance_included`(산재보험료 포함여부, 기본 true)/
  `industrial_insurance_rate`(별도일 때만 쓰는 요율%, 기본 0) 4개 컬럼 신규.
  **주의**: 작업지시서 원안은 최종 계산 총액을 담을 `driver_total_payout`
