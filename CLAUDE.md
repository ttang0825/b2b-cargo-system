...
**작성일: 2026-07-23** (최종 갱신: 2026-08-26, Claude Code 세션에서 직접 갱신 — 51차
...
15차 거리 구간 매칭 버그 수정 →
16차 운임 기준 교체 →
17차 차급 9종 확장 → 20차 전체(3-1·3-2·3-3·3-4) → **18차 이용약관 동의까지 완료**.
🔴 **차수 순서가 한 번 바뀌었다** —
...
9차=34차, 10차=35차, 11차=37차, 12차=39차, 13차=40차, 14차=43차, 15차=44차, 16차=45차, 17차=46차, 20차 3-4=47차, 20차 3-1·3-2·3-3=48차, **18차=49차** 세션 — 29차는
...
붙인 결과가 아래이며, **17차 · 20차 전체 · 18차 · 19차 · 21차까지 끝났다** —
...
🔴 **그래서 화주포털 화면 개편이 22·23차로 밀렸다** — 21·22차로 예고돼 있던 것이다.
🔴 **다음은 22차(화주포털 화면 ①)**이고, **③(anon GRANT 회수)은 미정**이다 — RLS 가 이미
막고 있어 추가 방어이며 급하지 않다. 아래 5번 첫 항목들을 볼 것:
...
- 🔴 **다음 지시서는 22차(화주포털 화면 ①)다** — 사이드바 8개 · 모바일 탭바 5개 · 홈 ·
  발주 요청 · 견적 확인. 그 다음이 **23차(화면 ②)** — 배차·운송 조회 · 정산 · 월별 통계 ·
  배송지·화물 관리 · 담당자 정보. 🔴 **9/7 을 기다리지 않고 바로 이어간다**(2026-08-26 확정)
  — 화주포털은 로그인한 화주만 보므로 공개 리스크와 무관하다.
- 🟢 **20차는 전부 끝났다**(3-4는 47차, 3-1·3-2·3-3은 48차. 운영 DB 반영까지 완료).
  ⚠️ **22·23차 화면이 그 위에 올라간다** — 배송지 담당자·프리셋·`도크`를 쓰는 화면이
  아직 없는 것은 **의도된 상태**다.
  🟢 **51차가 `customer_locations`·`customer_presets` 를 화주 롤로 실측해 뒀다** — 자기
  회사 것만 읽고 쓸 수 있다. 22차에서 "저장이 안 된다"가 나오면 정책이 아니라 화면 쪽이다.
- ⏸️ **가산기준 조정은 미정 차수로 밀렸다**(2026-08-26, anon 정리를 먼저 하기로 확정).
  **17차에 들어온 새 실측 때문에 범위가 늘어 있다.** 원래 예고분은 파손주의 `30,000→15,000` · 장척/중량
  ...
  - 🔴 **18톤 전 구간 ×0.89 조정 검토** — ⚠️ 46차 기록과 마이그레이션 주석은 이것을
    *"18차에서"*라고 적었는데, **차수가 밀려 이제 19차다**(내용은 그대로).
    2026-08-25 중대형 실거래 114건 중 18톤 완료
...
- ⚠️ **사용자가 배포본에서 확인할 것(18차 잔여)**: `/apply`에 체크박스가 **정확히 2개**이고
  ...
  신청을 한 건 넣은 뒤 `verify` ⑧-c에 **`terms / terms-v1`**이 뜨고 ⑧-b **고아 행 0**인지.
- ⚠️ **사용자가 배포본에서 확인할 것(16·17차 잔여)**: `/vehicles` 시작가 **48,000원부터** /
  ...
  확인할 수 없다. 🟢 **다만 47차부터 운임기준표·동의 데이터 자체는 `verify` 워크플로로
  세션이 직접 확인할 수 있다** — 남은 것은 "화면에 어떻게 보이는가"뿐이다.
