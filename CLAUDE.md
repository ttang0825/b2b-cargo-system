...
> 🔴 이 문서(`CLAUDE.md`)는 **374,517자**입니다(2026-09-20 기업고객 리워드 1차 기록 직후) —
> 컨텍스트 예산 판단에 쓰는 값입니다. ⚠️ 그 직전에는 365,053자, 그 앞에는 359,254자였습니다.
...
| `docs/history/current.md` | 이용가이드 재작성 #175 2026-09-18  →  기업고객 리워드 1차 2026-09-20   (6건) | 50,959자 |
...
```
  ...
  ——   기업고객 리워드 1차 · 내부 기반 (차수 없음)  ✅ 완료 (2026-09-20 · 커밋 **3개**(A·B·C
        장별) · 🔴 **DB 변경 있음**(마이그레이션 1건 · `_migrations` 45 → **46행**) ·
        신규 의존성 0 · 화주포털 0줄 · 공개 화면 0줄 · 랜딩 0줄 · 문자 0줄 · `lib/legal/` 0줄 ·
        `globals.css` **순수 추가**(+144 / −0) · 파일 16개 · 프리렌더 46 → **47** ·
        실사용 리뷰 라운드 **0**. 🔴 **차수를 붙이지 않았다**(로드맵의 「다음 = 24시콜 ⓑ」와
        ...
        **적립이 아예 안 나간다**(3초 상한 + `await` · 절대 던지지 않는다) ·
        ...
        🔴 **적립 기준은 기본 운임 공급가액 하나**다(`splitVat` · `Math.floor` 절사 ·
        요율은 캠페인에서 읽고 `earn_rate_snapshot` 에 남긴다) — 🔴 **1.1 로 직접 나누지 말 것** ·
        🚨 **선착불 제외의 근거는 「부담이 크다」가 아니다** — **기준이 될 금액과 시점이
        존재하지 않는다**(화주가 위캐리에 입금하지 않으므로 사건 자체가 없다 · HANDOFF §5-12).
        🔴 **그렇게 적지 않으면 다음 세션이 「요율을 낮춰서 넣자」로 간다** ·
        🟢 로컬 Postgres 16 복제 + 부정 시험 **5건이 전부 의도한 단언에서 멈추고 롤백** ·
        **PostgREST 만 흉내낸 목을 진짜 DB 위에 얹어** 컴파일한 원본을 그대로 돌렸다
        ...
        🚨 **사용자 몫 — 이벤트 안내 페이지를 고치기 전에는 링크를 보내지 말 것**
...
```
🔴 **기업고객 리워드 1차(2026-09-20) 이후 세션이 알아야 할 것만 요약** — 전문은 `docs/history/current.md` 맨 위:
```
  🔴 정책 0개       `reward_campaigns`·`reward_memberships`·`reward_ledger` 셋 다
                  **RLS on + 정책 0개**이고 서버 라우트 다섯이 유일한 통로다
                  (`/api/admin/reward/{summary,ledger,membership,adjust,accrue}`).
  ...
  🚨 트리거가 둘     건별 `invoices.payment_received` · 월정산 묶음 `payment_status='paid'`.
  ...
  🔴 적립 기준 하나  **기본 운임 공급가액** = `splitVat(청구총액 − 스냅샷에 섞인 추가비, 구분).supply`
                  → `Math.floor(× 요율)`. 🔴 **1.1 로 직접 나누지 말 것**(11,000원마다 1원) ·
                  🔴 **`Math.round` 가 아니다**(99,999 × 5% = **4,999**) ·
                  🔴 **요율을 리터럴로 적지 말 것**(캠페인에서 읽고 스냅샷에 남긴다) ·
                  🔴 **빼는 추가비는 그 정산 건 생성 시점에 이미 포함된 것만**이다
                  (35차 A-7 과 같은 세 규칙 — 그 뒤 것까지 빼면 **두 번 깎인다**)
  🚨 선착불 제외 근거 **「부담이 크다」가 아니다.** 선착불은 **화주가 위캐리에 입금하지 않아**
  ...
  ⚠️ 사용자 몫      🟢 **DB 는 merge 전에 apply 를 끝냈다**(`_migrations` **46행**) — 코드 밖에서
                  할 일 0 · 🚨 **이벤트 안내 페이지를 고치기 전에는 링크를 보내지 말 것**
  ...
  ⚠️ 안 한 것       화주포털 리워드 카드·적립내역(2차 · `portal_visible` 첫 독자) · 문자 안내(2차) ·
