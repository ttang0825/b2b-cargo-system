  ...
  return (
    <div ref={rootRef} className="landing-page" style={{ width: "100%", margin: "0 auto", overflowX: "clip", background: "#F4F3F0", color: "#0E0F12" }}>
      ...
      <section id="top" className="landing-hero"
        ...
        {/* 🔴 **배경이 `image-set()` 인 것은 선명도 때문이다 — `url()` 하나로 되돌리지 말 것**
            (2026-09-17). 사진 띠는 1600px 화면에서 약 1208px 폭이라 2912px 원본이 **0.5배로
        ...
        <div className="landing-hero-copy" style={{ position: "relative", maxWidth: "min(760px, 44%)" }}>
          ...
          {/* 🔴 **히어로에서 「무료 견적 문의」 버튼을 뺐다**(사용자 지시 2026-09-02).
              ...
              ⚠️ **2026-09-14 에 「운송관리 계정 신청」이 전화 아래로 들어왔다**(사용자 지시).
                 이 자리에는 *"히어로에 남는 유일한 행동이 전화다 · 새 버튼을 만들지 말 것"*
                 이 적혀 있었고 41차도 *"히어로에 세 번째 선택지를 다시 넣지 말 것"* 이라
                 적었다. 🔴 **그 두 줄은 이 지시로 뒤집혔다 — 그것을 근거로 이 버튼을 빼지
                 말 것.** 다만 **넷째는 넣지 않는다**(견적은 헤더 CTA·마감 CTA 에 있다).

              🔴 **세로로 쌓는다**(사용자 지시 「전화문의 버튼 아래에」) — 가로로 두면
                 히어로 본문이 `min(760px, 44%)` 라 `minWidth: 330` 짜리 둘이 못 들어가
                 줄이 접힌다. 모바일은 `landing.css` 가 이미 같은 방향으로 쌓고 있었다.
              🔴 **계정 신청은 옐로다** — 랜딩의 다른 세 곳(⑥ 운송관리 섹션 · 마감 CTA ·
                 FAQ)이 전부 `#FFD834` 라, 여기만 다른 색이면 같은 행동으로 안 읽힌다.
                 전화가 검정 채움 **+ 그림자**로 여전히 1순위이고 이 버튼엔 그림자가 없다.
              🔴 **`?from=landing` 을 붙이지 않았다** — 그 파라미터는 「페이지 한참 아래
                 ⑥ 섹션에서 왔을 때 뒤로가기가 맨 위로 튕기지 않게」 하려는 것이다
                 (`BackToHomeLink`). **히어로가 곧 맨 위**라 여기 붙이면 얻는 것이 없다. */}
          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-start", gap: 12, marginTop: 34 }}>
            ...
            <Link href="/apply" style={heroBtn("#FFD834", "#0E0F12", "#FFD834")}>
              운송관리 계정 신청
            </Link>
          </div>
        </div>
      ...
      <section className="landing-cta"
        ...
        {/* 🔴 **운영시간을 이 화면에서만 뺐다**(사용자 지시 2026-09-02).
            🔴 **되살리지 말 것** — 2026-09-18 에 사용자가 *「랜딩페이지는 없는 채로
               유지하자」* 로 다시 확정했다.
            ⚠️ 상수 자체는 지우지 말 것. 같은 날 **둘로 갈렸다** —
               `COMPANY_SUPPORT_HOURS`(18시)는 공용 푸터·`/apply`·JSON-LD 가,
               `COMPANY_SUPPORT_HOURS_PORTAL`(19시)은 화주포털 셸·로그인 화면·포털
               이용가이드가 쓴다. 랜딩은 **둘 다 안 그린다.**
