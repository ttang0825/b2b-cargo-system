...
export async function POST(req: Request) {
  ...
  // ⚠️ **화주포털 푸시를 여기서 보내지 않는다**(2026-09-16 사용자 확정) —
  //    *「화주포털 알림중 정산관련해서는 알림이 안뜨는게 좋겠다」*. 🔴 되살리지 말 것:
  //    화면 안 알림(`lib/portalAlert.ts` 의 `invoices`)도 같이 없앴으므로 푸시만
  //    되살리면 **폰으로는 오는데 화면에는 안 뜨는** 상태가 된다.
