...
export default function ApplyStatusPage() {
  ...
  async function handleSearch(e: React.FormEvent) {
    ...
    setError(null);
    try {
      const res = await fetch("/api/apply-status-lookup", {
        method: "POST",
      ...
      <main className="container" style={{ maxWidth: 640, paddingTop: 40 }}>
        <div className="page-header">
          <div>
            ...
            <p className="page-desc">등록 신청 시 입력하신 연락처로 진행 상황을 확인하실 수 있습니다.</p>
          </div>
        </div>
        ...
        {searched && (
          <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
            {results.length === 0 ? (
              <div className="card">
                <div className="empty-state">이 연락처로 등록된 신청을 찾을 수 없습니다.</div>
              </div>
            ) : (
              results.map((r) => (
