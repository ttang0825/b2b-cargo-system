...
// ⚠️ **「`public/customer/sw.js` 에는 넣지 않는다」고 적어 둔 줄은 2026-09-16 에
//    뒤집혔다** — 사용자가 화주포털에도 붙이기로 확정했다(*「견적과 배차,운송 정산만」*).
//    🔴 **그쪽 핸들러를 그 줄을 근거로 지우지 말 것.** 다만 **보내는 사건과 문구는 서로
//    전혀 다르다** — 이쪽은 새 접수(직원), 그쪽은 자기 거래의 진행(화주)이다.
//    🔴 **구독 표도 갈려 있다**(`push_subscriptions` / `customer_push_subscriptions`) — 합치지 말 것.
//    ⚠️ 그래서 이 두 파일은 **푸시 이후 부분부터 위쪽 주석의 「두 파일은 같다」에서 갈라졌다**
//    — 캐시 규칙까지는 여전히 같으니 **그쪽을 고칠 때는 반드시 둘 다** 고칠 것.
// ─────────────────────────────────────────────────────────────────────────────
