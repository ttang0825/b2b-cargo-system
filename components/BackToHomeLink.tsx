...
//    - 랜딩에서 왔는지는 **`?from=landing` 쿼리 파라미터**로 판정한다. 그 표시가 없으면
//      평범한 링크로 `/`(맨 위)로 간다.
//      🔴 **지금 그 파라미터를 붙이는 곳이 저장소에 하나도 없다**(2026-09-08 실측 grep).
//      13차에 만들 때는 `app/page.tsx` ⑥ 버튼 2개와 `components/LandingHeader.tsx`(37차)가
//      붙였는데, **30차가 랜딩을 새 시안으로 통째로 갈아엎으면서 그 버튼들이 없어졌고**
//      37차 헤더는 2026-09-08 에 삭제됐다. 즉 이 링크는 현재 **항상 `/` 맨 위로 간다.**
//      🔴 「왔던 자리로 복원」을 되살리려면 랜딩이 두 화면으로 보내는 링크에 `?from=landing`
//      을 다시 붙이면 된다 — 이 컴포넌트는 고칠 것이 없다.
