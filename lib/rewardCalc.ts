...
//
//   적립률      운임 **공급가액**(부가세 제외)의 5% · 🔴 **100원 미만 내림**
...
/**
 * 🔴 **적립 단위 — 100원**(사용자 확정 2026-09-21).
 *
 * ⚠️ 그전에는 **1원 단위 절사**였다(2026-09-18 확정). 🔴 **1원으로 되돌리지 말 것.**
 */
export const REWARD_EARN_UNIT = 100;

/**
 * 공급가액 × 요율 → 적립액. 🔴 **100원 미만 내림**(`Math.round` 가 아니다).
 *
 *   100,000 × 5% = 5,000        (딱 떨어지면 그대로)
 *    99,999 × 5% = 4,999.95  → **4,900**
 *    72,727 × 5% = 3,636.35  → **3,600**
 *
 * 🚨 **「표시만 100원 단위」로 만들지 말 것.** 적립액 자체를 100원 단위로 낸다 —
 *    화면에서만 내리면 **원장 합계와 화면의 잔액이 갈린다**(원장은 1원 단위인데
 *    화면은 100원 단위라, 건이 쌓일수록 어긋남이 커진다). 원장에 들어가는 값이
 *    이미 100원 배수이면 그 합도 100원 배수라 **두 숫자가 영원히 같다.**
