...
//
// 🔴 **줄바꿈(`\n`)이 들어 있고, 읽는 쪽 `<p>` 에 `white-space: pre-line` 이 걸려 있다.**
//    빼면 세 줄이 한 문단으로 붙어 다시 길어 보인다(그것 때문에 줄인 것이다).
//    팝업(`RatesModal.tsx`)·`/vehicles` **두 곳 다** 걸려 있어야 한다 — 한쪽만 빼면 갈린다.
//
// ⚠️ **2026-09-02 에 셋째 줄(고정물량)이 더해져 네 줄이 됐다.** 필수 넷은 그대로다.
//
...
//
// ⚠️ **혼적(합짐) 문장을 뺐다**(사용자 지시 2026-09-02 — 105자가 너무 길다). 「±」 한
//    글자가 오르내림을 다 말하므로 필수 넷은 그대로 살아 있다. 다만 **내려가는 경로가
//    무엇인지는 화면에서 더 이상 말하지 않는다** — 실제로는 혼적 할인 하나뿐이고,
//    견적 자동계산이 `applyMixedDiscount()` 로 최종금액에서 차감한다
//    (`app/admin/quotes/page.tsx`). 🔴 되살리려면 둘째 줄에 붙일 것.
// ⚠️ 「최소 운임」이라는 말은 쓰지 않는다 — 「기준가」(오르내린다)와 뜻이 부딪힌다.
// 🔴 조건 없는 「할인」·「더 저렴」·「최저가」를 쓰지 말 것(30차 금지 표현) — 「±」는 범위를
//    말할 뿐이라 무관하다.
// 🔴 **셋째 줄(고정물량)에 「더 저렴합니다」·「할인」이라고 쓰지 말 것**(2026-09-02 추가).
//    조건·기간 없는 할인 표시는 금지이고, 실제로도 반복 물량은 노선·조건을 정해 **따로
//    산정**하는 것이지 정률 할인이 아니다. **「별도로 안내」가 사실이면서 안전하다.**
//    ⚠️ 근거는 화주포털의 배송지·화물·요청 프리셋(`customer_presets`)과 서비스 카드의
//       「고정화물 배차」다 — 없는 기능을 적은 것이 아니다.
//
// 🔴 **2026-09-02 에 넷째 줄을 표 아래로 내렸다**(사용자 지시) — 「정확한 금액은
//    견적으로」는 표를 **보고 난 뒤** 읽어야 뜻이 산다. 그래서 상수가 둘이 됐다.
//    ⚠️ **필수 넷은 여전히 넷 다 화면에 있다** — 자리만 위·아래로 나뉜 것이다.
//    🔴 둘 중 하나를 지우면 ③ 또는 ④ 가 화면에서 사라진다. 두 곳을 같이 볼 것.
export const START_PRICE_NOTE =
  "10km 이내 기준 금액이며 부가세는 별도입니다.\n" +
  "실제 운임은 조건에 따라 기준가에서 ± 됩니다.\n" +
  "정기적으로 반복되는 물량은 조건을 정해 별도로 안내드립니다.";

/** 기준가 표 **바로 아래**에 붙는 한 줄(위 ③·④).
 *  🔴 팝업과 `/vehicles` 가 같은 상수를 읽는다 — 화면에 직접 적지 말 것.
 *  🔴 표를 못 그린 경우에는 이 줄도 그리지 않는다 — 그때는 「기준가를 불러오지
 *     못했습니다. 정확한 금액은 견적으로 안내드립니다.」가 같은 말을 이미 한다. */
export const START_PRICE_FOOTNOTE =
