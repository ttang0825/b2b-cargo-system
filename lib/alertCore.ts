...
//      포털    `lib/portalAlert.ts`         (둘: 견적 · 배차 — 배지는 넷이다)
...
// ── 늘어난 것만 고르기 ─────────────────────────────────────────────
//
// 🔴 **화면 안에 묻어 두지 않고 순수 함수로 뺀 것은 재어 보기 위해서다.**
...
// ── 화면에 뜬 배너 치우기 ───────────────────────────────────────────────────
//
// 🔴 **윈도우·폰 알림을 눌러 그 화면으로 들어오면 창 안의 배너는 할 일을 다 한 것이다.**
//    남겨 두면 이미 보고 있는 것을 또 가리키는 꼴이라 손으로 닫아야 한다
//    (사용자 요청 2026-09-16 — *「해당 알림(창내 배너)은 자동으로 꺼지게」*).
//
// 🔴 **화면을 옮길 때만 부를 것.** 배너가 새로 뜰 때마다 부르면 **그 화면을 열어 둔
//    사이에 도착한 소식을 영영 못 알린다** — 화주포털은 목록이 스스로 다시 그려지지
//    않아서 「배지는 눌러 두고 배너는 띄운다」로 정한 자리다. `useEffect(..., [pathname])`
//    로 걸면 화면이 바뀔 때만 돈다.
//
// 🔴 **탭 제목(`unseen`)은 건드리지 않는다** — 그쪽은 탭이 보이는 순간 0이 되고,
//    알림을 누르면 창이 앞으로 나오므로 이미 처리된다. 여기서 같이 빼면 두 번 빠진다.

/** 지금 화면이 그 배너가 가리키는 곳이면 뺀다. 뺄 것이 없으면 **받은 배열을 그대로** 돌려준다. */
export function dropToastsForPath<T extends { href: string }>(
  items: T[],
  pathname: string | null
): T[] {
  if (!pathname) return items;
  // 상세 화면(`/admin/quotes/123`)도 그 목록에 들어온 것으로 본다.
  const keep = items.filter((i) => !(pathname === i.href || pathname.startsWith(i.href + "/")));
  // 🔴 같으면 **같은 배열을 돌려준다** — 새 배열을 만들면 화면이 매번 다시 그려진다.
