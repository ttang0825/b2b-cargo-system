 ...
 *
 * 🔴 **문의 줄을 넣지 않는다 — 열세 판본 중 이것만 예외이고, 사용자가 확인한 예외다.**
 *    ⚠️ 2026-09-23 지시는 *「모든 문자메세지의 마지막에 문의 부분에 담당자의 연락처를
 *    남기자」* 였지만, 이 문자에는 애초에 「문의 부분」이 없고 한 줄을 더하면 요금 구간이
 *    바뀐다. 실측해서 보고했고 **사용자가 「SMS 로 남긴다」를 골랐다.**
 *
 *      「문의 1661-2403」(대표번호)          +15byte  →  102byte  LMS
 *      「문의 010-4147-3327 (담당 전준영)」  +33byte  →  120byte  LMS
 *
 *    🔴 **「모든 문자」를 근거로 넣지 말 것.** 대신 ① 이 문자의 **발신번호가 담당자 본인
 *    번호**라(35차) 받는 사람이 그대로 회신·통화할 수 있고 ② 담당자 연락처와 대표번호가
 *    **견적서 화면 안에** 있다.
 *
