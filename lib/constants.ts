...
// 🔴 **차급은 9종으로 끝나지 않는다 — 더 늘어난다는 전제로 만들어져 있다.**
//
// 2026-08-25 중대형 실거래 114건(24시콜)에서 **8톤 13건 · 9.5톤 5건 · 14톤 12건 ·
// 15톤 6건**이 나왔다. **114건 중 36건(32%)이 이 배열에 없는 차급**이다.
// 한국 화물시장 표준 차급은 `1 / 1.4 / 2.5 / 3.5 / 5 / 5톤축 / 8 / 11 / 15 / 18 / 25`이고
// **v8이 8톤과 15톤을 빠뜨렸다.** 이 둘을 넣으면 커버율 100%가 된다
...
//      `migrations/2026-08-25_rate_tiers_9types.sql`을 본보기로 삼으면 된다.
...
// ────────────────────────────────────────────────────────────────────
//
// ⚠️ **18톤 값은 잠정값이다** — 위 실측에서 우리 표가 약 13% 높은 것으로 나왔다
// (18톤 완료 8건 역산). v8 보간식 `11톤 + 0.70 × (25톤 − 11톤)`의 위치계수 0.70이
// 과대평가였고, 실측에서 18톤은 11톤과 거의 같은 가격대다(차주지급 중앙값 둘 다
// 180,000원). 🔴 **18차에서 전 구간 ×0.89 조정을 검토한다.**
