...
//
// 사용자 확정:
//   ① ~~확인을 한 번 더 받는다(연락처 뒤 4자리)~~ → 🔴 **같은 날 없앴다**(아래)
//   ② **유효기간이 지나면 막는다**
//   ③ **기존 견적안내 문자를 대체한다**
//
// ⚠️ **뒤 4자리 확인은 2026-09-15 에 사용자 지시로 제거됐다**
//    (*"뒤4자리 확인을 빼고 바로 링크를 확인할수 있으면 좋겠다"*).
//    함께 있던 `matchesPhoneLast4()` 와 실패 문구도 같이 지웠다.
//    🔴 **되살리려면 사용자에게 먼저 물을 것** — 한 번 뺀 것을 말없이 되돌리면
//       「왜 갑자기 번호를 묻느냐」가 된다.
//
// 🔴 **이 파일이 「누가 이 견적서를 볼 수 있는가」의 유일한 정의처다.**
//    토큰 생성과 유효기간 판정이 여기 있다. 라우트나 화면에 같은 판단을 다시 적지
//    말 것 — 한쪽만 고치면 **막았다고 생각한 문이 열려 있다.**
//
// 🔴 **토큰이 곧 열쇠이고, 지금은 유일한 방어선이다.** 이 값을 아는 사람은 그 견적서
//    (화주 상호·구간·금액)를 본다. 문자가 전달되면 받은 사람도 그대로 열 수 있다 —
//    그것이 사용자가 고른 절충이다.
