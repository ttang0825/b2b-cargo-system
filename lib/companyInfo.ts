...
// 인사말(대표 인사) 본문.
// 🔴 **2026-09-08 부터 이 네 상수를 읽는 화면이 없다** — 유일한 사용처였던 회사소개
//    (`/about`)를 화면째 삭제했기 때문이다(사용자 확정 — "나중에 필요하면 새로 다시
//    만들면 된다"). 🔴 **그래도 지우지 말 것** — 이 인사말 초안이 남아 있는 곳은 여기뿐이고
//    (그 외에는 git 이력뿐), 회사소개를 다시 만들 때 이 상수들을 그대로 읽으면 된다.
//    「안 쓰이니 정리하자」로 지우면 초안을 다시 쓰는 일이 된다.
// ⚠️ **이 문구는 확정본이 아니라 자리를 잡기 위한 임시안이다.** 이후 다듬어 교체할 예정이며,
// 교체할 때는 이 상수들만 고치면 회사소개 화면에 그대로 반영됨(문단 단위로 렌더링).
