...
/**
 * 견적이 「이제 운송오더를 만들어야 하는 상태」임을 뜻하는 **DB 값**.
 *
 * 🔴 **문자열 `'수주'` 를 다른 곳에 다시 적지 말 것** — 원래 `lib/unlinkedWonQuotes.ts`
 *    에 있었는데, 그 파일이 맨 위에서 **관리자 클라이언트(`lib/supabaseClient.ts`)를
 *    import** 해서 화주포털 코드가 가져다 쓰면 포털 번들에 관리자 세션 클라이언트가
 *    딸려 들어간다(원칙 1번 — admin/customer 는 반드시 별도 클라이언트).
 *    그래서 **의존성이 0인 이 파일로 옮겼고** 저쪽은 그대로 재수출한다.
 */
